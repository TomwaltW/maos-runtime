...
ADDED_PATTERNS: dict[str, tuple[str, ...]] = {
    HANDOFF_ANGER: (
        ...
        # （「破东西 / 破包装」多半在说商品本身，不收）
        r"(?:破|烂|狗|鬼|渣)(?:客服|机器人|系统|平台|服务|店铺|公司|态度|app|软件|回复)",
        ...
        "神经病", r"(?:你们?|客服)(?:是不是|是)?有病", r"(?:客服|你们?)(?:就是|是)?(?:个|一个)?摆设",
        r"(?:客服|你们?)(?:是)?死人", "听不懂人话", "不说人话",
    ),
    ...
    # 条件（不 / 没 / 再 / 要是 / 如果 / 否则 / 不然）与后果之间必须有「就 / 否则 / 不然」这类连接：
    # 「我不会给差评的」「不退款我就自己留着」「不退的话我会很失望」都不算（纯发泄、拿主意、表态）。
    HANDOFF_COMPLAINT: (
        r"(?:不|没|再|要是|如果|若是?|否则|不然)[^。.!?！？]{0,16}?(?:就|否则|不然)[^。.!?！？]{0,8}?"
