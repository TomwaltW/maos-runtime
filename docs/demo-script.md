...
**本分镜按屏幕上真能看到的东西写。** 每一镜的命令都在 `147df03`（整合轮 5，含 Y-4）上
真跑过，输出是实贴的，耗时是实测的。念词里没有一句是屏幕上找不到的 —— 这是本文件唯一的判准。

---

## 逐镜耗时表（`147df03` 实测，2026-08-29 · 已含 Y-4）
...
| 5 | 02:06 — 03:01 (55s) | 无（回看主终端） | — | 190 | 47.5s | +7.5s |
| 6 | 03:01 — 03:39 (38s) | 无（回看主终端） | — | 131 | 32.8s | +5.2s |
| 7 | 03:39 — 04:12 (33s) | `verify.py` | 0.11s | 111 | 27.8s | +5.1s |
| 8 | 04:12 — 04:40 (28s) | `git diff --stat` | 0.02s | 101 | 25.2s | +2.7s |
| | **合计** | | **1.07s** | **960** | **239.9s** | |

**总长 4 分 40 秒（280s）**，在 `docs/submission-checklist.md` C 段要求的 3–5 分钟内。

镜 5 是本轮唯一改动过的镜：整合轮 5 合入 Y-4 后它从「问不出终态就转人工」
变成「先换一次渠道、再停手」，念词 139 字 → **190 字**，分配 40s → **55s**，
其后镜 6/7/8 各顺延 15 秒。总长 4:25 → **4:40**。
...
已按 `147df03` 逐条复核）：Element 房间未接通、审批由场景脚本自动驱动。
本分镜按**当前代码的真实行为**写，不按手册想象写。

> ✅ 原来的第三处「换渠道 replan 演不出来」**已在整合轮 5 解除** —— Y-4 合入后
> 场景 7 真能演，见镜 5。前置也已从 6 条收敛到 **5 条**（Y-3 让 `make_evidence.py`
> 缺省一并产 R5）。**录制当天仍以实跑为准。**
...
🔴 **屏幕上不止七行。** 实测这七行之间夹着 **11 条 `· warn:` 缩进行**
（`trace-tree` 下 6 条、`business-outcome` 下 4 条、`authoritative-fact` 下 1 条）。
读数变过两次：17 条（整合轮 5 前）→ 10 条（Y-1/Y-2 各消掉一类）→ **11 条**
（Y-4 让场景 7 真收到过一次回执，新增 `有回执但 biz_status 不是 settled` 一行）。
**别照抄旧读数**；那一行是**预期内**的，判定仍 7/7。
...
| 2 | **网关错误码 → replan 换渠道** | ✅ **已能演。** 整合轮 5 合入 Y-4（`783d9dd`）后，实跑 `python3 run.py --scenario 7`（`147df03`）屏幕上有完整的换渠道段：`disposition=replan_channel` → `s7-primary -> s7-backup` → 状态轨迹里的 `gate_rework` / `requeue`，收口行打 `换渠道重试: 1 次 replan`。`test_replan_gateway.py` 仍 **19 passed** | 02:06 那一镜 | **照该镜念，输出已是实测值。** 唯一红线换了方向：**别念成「重试到上限」** —— 只 replan 了 1 次，上限是 2 |
...
九轨已全部合入，本文件八镜的屏幕输出与念词**本轮已全部对齐实测**。剩下的只有：

| # | 事项 | 等什么 |
| :-- | :-- | :-- |
| 1 | 视频官方规格（时长上限 / 分辨率 / 格式 / 大小 / 字幕） | 见 [`docs/open-questions.md`](open-questions.md) **OQ-2** |
| 2 | 录制当天现跑对屏 | 每一镜的输出都要当天实跑一次核对；对不上以当天为准重贴，**不许照本文件念** |
