...
| 日期 | Phase | 现象 | 影响 | 建议处理时机 |
|---|---|---|---|---|
| 2026-09-24 | p13 | 旧库探针的 PG 分支（读 pg_constraint）本容器没有 PG，只用假连接测了分派与规范化文本的拆法，没在真库上跑过 | PG 上 pg_get_constraintdef 的实际输出若与假连接的形状不同，探针可能漏报（放行旧库）或误报 | p13 整合期或作者 Mac 上补一条 PG 门控测试：真库建 p12 形状的 cs_turn，断言 ensure_schema 抛 CsSchemaOutdated |
| 2026-09-24 | p13 | 观察取号只在**同进程**内不重号（持 store 锁的事务）；两个进程并发落同一轮观察时，会各自数到同一个 n，后一个撞主键抛 IntegrityError（不重号，但这一轮失败） | 单进程 run_ingress 不触发；多进程 / 多实例部署时偶发一轮查单结果落不下 | 多实例部署前：取号撞主键时重试一次，或 PG 上用 advisory lock |
| 2026-09-24 | p13 | ensure_schema 每个入口都跑整份 DDL，p13 起再加两次只读探针；库是旧形状时每条消息都会抛 CsSchemaOutdated | 旧库在 desk 的「永不抛」兜底下会表现为「每轮都转人工」，而不是启动即失败 | T174 装配 run_ingress 时在启动处先调一次 ensure_schema，让旧库在启动时就报「要重建」；每入口一次 DDL 的开销按 store 缓存留给 p14 |

## task-t172（外部端口：查单与退款预检，2026-09-24）

| 日期 | Phase | 现象 | 影响 | 建议处理时机 |
|---|---|---|---|---|
| 2026-09-24 | p13 | **ToolInvoked.detail.error 里有查单异常原文**：invoke_tool 在工具抛异常时写「类名: 原文」，MockOrderSystem 的 KeyError 原文里列着账本上全部订单号（真平台的 CommerceError 里也可能有别的单号）。cs_ports 的出参与日志已经不带，event_log 里仍有 | 审计行里躺着与本轮客户无关的订单号（R5 增量要求订单号不进 event_log） | p14：invoke_tool 只落异常类名（或按工具声明脱敏），改完补一条全仓 ToolInvoked 不含原文的守卫 |
| 2026-09-24 | p13 | **订单系统登记表是进程全局的**：CS 查单前重登记，但 run_payload 的清表若恰好落在「重登记」与「invoke_tool」之间（两个线程并发），这一次查单读到 LookupError | 落 system_misconfigured、转人工（方向安全，只是一次误转） | p14：给工具侧登记表加按名注册 / 不整表清空的接口，或 run_payload 只清自己登记的名字 |
| 2026-09-24 | p13 | **CredentialMissing 落 platform_error**：契约的异常顺序里没有它，按「其它异常」处理；「没配凭据」其实是装配问题 | 运营统计里「平台出错」混进了「我们没配钥匙」，靠 error_kind 类名才分得开 | p14 契约修订时考虑把 CredentialMissing 归 system_misconfigured |
| 2026-09-24 | p13 | **查单结果不核对返回的订单号是否就是查的那一单**（order.query 返回 dict 里的 order_id 与 query_key 不比） | 适配器解析错位时可能把别的单的状态说给客户听（现有七个平台都按请求的键取回同一单，未见实例） | p14：在 cs_ports 或 order.query 层加「返回的 order_id 必须等于查询键」的失败即关校验，先确认各平台 id 的规范化形状 |
| 2026-09-24 | p13 | **预检卡上的裁定按客户说话那一刻算，内部同事发 /refund 时按发送时刻重算**：command_line 形如 /refund 单号 原因码，不带日期 | 卡片挂得久、又恰好卡在政策窗口边界时，发出去的预检结论可能与卡上不同（以发出时为准，方向安全） | p14：卡片上写明「按何时算」，或 command_line 带上申请日期参数 |
| 2026-09-24 | p13 | **查单在客户对话里同步阻塞**：真平台最坏约 2 次 × 5 秒 + 1 秒退避 | 平台慢时这一轮回复要等十秒上下 | p14：前台查单改异步或给一轮对话设总预算 |
| 2026-09-25 | p13 | **run_requests.build_case 按单号在全表取最高版本、不看租户**（复核 L2-2 追出来的存量行为）：底账里两个租户有同号订单时，/refund 自己也会按版本更高的那个租户的行与政策裁定。CS 预检已失败即关（有别的租户同号行就拒），/refund 那条路没管 | 多租户底账上内部同事发 /refund 可能拿到别的租户的裁定；现有演示底账只有一个租户，未见实例 | p14：build_case 按申请人所在租户过滤，或底账校验时拒绝跨租户同号 |
| 2026-09-25 | p13 | **同一电商平台只能配一家店**（复核 L3-1）：MAOS_CS_ORDER_SYSTEMS 的 JSON 里同平台第二家在装配期就被拒，因为现有适配器的店铺身份与凭据都读进程级环境变量 | 一个租户在同一平台开多家店（或 tiktok_shop 一个授权下多个 shop_cipher）时，前台只能查其中一家 | p14：适配器按店铺分凭据（凭据名带系统名，或 account 参与请求路由），再放开同平台多家 |
...
| 2026-09-25 | p13 | 名词表按收词规矩从 71 条撤到 31 条：「这款气炸锅包邮吗」「垃圾袋什么时候发货」「垃圾桶的盖子裂了」「相机的曝光补偿在哪调」「安装要另收人工费吗」「摇滚风的外套」等 13 句（NOUN_EDGE_T173 后半）回到 p12 的误转人工 | 普通咨询多转一次人工（与 p12 相同，不漏转） | 只能逐条加审过的、在触发词外至少多两个字的名词；或 p14 在真模型路径上对「只有一处触发词、且落在常见商品名里」的句子做二次判断 |
| 2026-09-25 | p13 | 裸的「不 + 诉求词」只在客户这一方开口时算撤回：「都想好了不退了」「考虑再三不退款」这类前面既不是分句开头也不是我 / 这单 / 算了的说法，不算撤回，上一轮的 refund 保留 | 前台照旧往退款那条路走（出预检卡、等客户确认），客户要再说一次「不用退了」 | 主会话量留出集时看撤回句的漏认率；要放宽就往 _OWN_SUBJECT_RE 里加拿主意的说法（想好了 / 考虑了），不要去掉这层 |
| 2026-09-25 | p13 | 没有单号字眼、单号后面紧跟品类名词的句子（「A1001手机上显示已签收」「A1001款到哪了」）照旧按型号不认（与 3831987 同） | 这一轮 order_no 为空，前台追问单号 | T174 接线时：会话里已绑定的单号出现在句中就直接认（绑定表是最终裁判），不靠理解层的形态判断 |

## task-t175（校验、守卫白名单与 p13 评测集，2026-09-24）

| 日期 | Phase | 现象 | 影响 | 建议处理时机 |
|---|---|---|---|---|
| 2026-09-24 | p13 | evaluate.turn_observations 直查 cs_observation、test_cs_eval_runner_t175 的 FakeDesk 也按契约 §1.3 的列直写这张表（W-A 期骨架里没有它，测试自建 IF NOT EXISTS） | T171 的真表若列名 / NOT NULL / CHECK 与契约有出入，合流后 p13 跑批读不回观察或测试写不进 | W-A 合流时对一遍；可改成调 T171 的 observations_for_turn |
| 2026-09-24 | p13 | check_reply 起认英文状态说法（shipped / delivered / arrive 各词形 / has been / will ship / in transit / is paid 等，否定不豁免） | T174 的英文固定话术（兜底 / 追问 / 过渡语）只要含这些词就会被拦成 unverified_claim | T174 写英文话术时照 T169 那条测试逐句过真校验器 |
| 2026-09-24 | p13 | 中文否定（「尚未发货」）放行、英文否定（「has not shipped yet」）拦下，口径不对称 | 无安全风险（英文更严）；前台英文不能说「还没发货」 | p14 由主会话统一口径 |
| 2026-09-24 | p13 | 守卫的「按真实出处」只对模块 / 函数 / 类有效：白名单模块再导出的数据对象（dict、实例）没有可信出处，按名字放行 | 例如名单模块若把注册表 dict 转手出去，from 名单模块 import 它判不到（task-t170 记的字面量局限照旧） | p14 |
| 2026-09-24 | p13 | p13 开发集没有英文触发词用例（T173 的英文词表本期才定） | 英文转人工路径没被开发集覆盖 | 整合期或 p14 按 T173 定下的词表补 |
| 2026-09-25 | p13 | check_reply 还不认中文口语状态说法：在路上了、已经发了、正在派送中、运输中、明天就能到、退款成功、已成功取消、订单已被取消（空观察下放行；p13 评测侧已认、记 off_table） | 若前台话术或模型回复出现这些，出门校验放行，只有评测能发现 | p14 由主会话统一中文同义说法口径后并进 check_reply |
| 2026-09-25 | p13 | 英文时限按时长本身判（within 24 hours 不要求紧接到账 / 发货动词），比中文严 | T174 的英文过渡语不能写「A colleague will reply within 24 hours」一类说法，中文「24小时内联系您」却放行 | T174 写英文固定话术时逐句过真校验器；p14 统一中英口径 |
| 2026-09-25 | p13 | 守卫对非 maos、非仓库一方代码的普通 import（第三方包、解析不了的顶层名）仍放行；本域子树的名字不要求解析 | cs 代码 import 第三方包判不到（今天 cs 代码只用标准库与 maos）；p12 §2.3 写的是「名单 + 标准库」，p13 契约只写了 maos.* 白名单 | 主会话定口径后改成：顶层名须在标准库名表（sys.stdlib_module_names）里，或是 maos 白名单名（复核实测现有 cs 代码不会误报） |
| 2026-09-25 | p13 | 退款桥轮若说了与夹具一致的措辞表那句，三项指标都不判 | §2.6d 要求退款桥轮一个状态字都不说；契约的 wrong_status 定义是「与夹具对不上」 | p14 评测批量化时考虑按期望（该轮该不该说状态）判 |
| 2026-09-25 | p13 | 评测侧的中文口语状态表只对话术库 script 校过不误报，T174 的固定话术还没写 | 若 T174 的中文固定话术恰好含这些说法（如「已经发了退货地址」），评测会记成 wrong_status | W-B 跑 p13 评测时对照；误报就收紧那一条模式 |
| 2026-09-25 | p13 | check_reply 新认「已完成 / 已为您完成」「已经到了」与英文 received your payment、will receive it + 时间 等（复核 L3R2-4） | T174 的过渡语若写「已为您完成登记」「Once we have received your payment」一类，出门校验会拦成 unverified_claim | T174 写固定话术时逐句过真校验器；误伤就收紧那一条（加否定前瞻），不放宽整张表 |
| 2026-09-25 | p13 | 守卫对非 maos 的普通 import 仍只按黑名单补（第二轮复核又找到一处：入口脚本目录里的裸名，本轮按黑名单式补上） | 同类裸名绕法每轮都可能再冒出一个；「标准库 + 名单」失败即关一次收口 | 主会话优先定口径（上一条同类 BACKLOG 的升级） |
| 2026-09-25 | p13 | 守卫里星号绑出的名字按测试进程里那个模块当下的名字空间算 | 运行时进程多加载了的子模块（包的名字空间里多出来的名字）判不到；本域模块今天都有显式 import，影响限于包的星号 import | p14 若要收紧：扫描范围里星号 import 一律禁（本域今天没有星号 import） |

## task-t174（前台编排 p13，2026-09-25）

| 日期 | Phase | 现象 | 影响 | 建议处理时机 |
|---|---|---|---|---|
| 2026-09-25 | p13 | p13 评测集 CS13-035 第 1 轮「嗨，客服小姐姐在吗」期望 answer / GEN-001，与 p12 触发词地板「客服小姐姐」→ requested 冲突；后两轮（含一轮 say=cancelled）随之 silent | 整份开发集的 wording_accuracy 只有 0.89，达不到文件门槛 1.0；maos/tests/test_cs_eval_p13_t174.py 把整份 meets 标了 xfail(strict)，去掉 CS13-035 后全对 | 整合期主会话裁定：改评测句（去掉触发词，推荐）或改触发词地板（动 p12 口径）；改完摘掉 xfail |
| 2026-09-25 | p13 | cs.answer 的可选入参 intent_hint 没写进 input_schema | docs/skill-catalog.md 看不到这个键 | 整合期补进 input_schema 并跑 gen_docs 重产 |
| 2026-09-25 | p13 | 「要不要查单」的判据有一截在前台（desk.order_need 的查询说法表与只报单号的判定），与理解层的诉求 / 进度线索分处两地 | 两份词表会漂：理解层补了说法、前台这边不知道 | p14 把「本轮要看具体订单」收进理解层的输出（Understanding 加字段要改契约） |
| 2026-09-25 | p13 | 外部渠道只有附件、没有字的消息，前台按空文本走兜底，计一次连续兜底 | 客户连发两张破损照片就被 repeated_fallback 转人工，卡片里看不到图（附件按 R1 不入库） | p14 定客服侧附件口径（是否入库、给人工看的引用） |
...
| 2026-09-25 | p14 | cs_eval 的 stderr 打码只接管 maos 这一支 logger；maos 以外命名空间的 logger、warnings 模块、第三方库直接写 stderr 的输出不在接管范围内（当前前台链路上没有这类输出） | 以后前台链路若引入别的命名空间日志并带原句，留出集可能从 stderr 外泄 | 整合期或引入新依赖时复查 |

## task-t181（p14 盲写留出集，2026-09-25）

| 日期 | Phase | 现象 | 影响 | 建议处理时机 |
|---|---|---|---|---|
| 2026-09-25 | p14 | p13 契约 §2 第 6c 步：换货诉求带单号、查单 ok 时会直接报订单状态走 answer（第 6d 步只对 refund / return 开预检），契约没说换货该不该出卡 | 换货客户拿到一句状态而不是转人工；本留出集因此没出这一类 | 整合期主会话定口径（换货是否并进退款桥或 needs_order_lookup），再补题 |
| 2026-09-25 | p14 | 「要退款」这一轮的 intent（refund_payment / return_exchange）只能从 p12 编号目录推，契约没有一句话写死 | 留出集与实现若各推一边，intent 读数会系统性差几轮 | 整合期量留出集时若 intent 失分集中在这类，先定口径再判 |

## task-t182（理解层泛化，2026-09-25）

| 日期 | Phase | 现象 | 影响 | 建议处理时机 |
|---|---|---|---|---|
| 2026-09-25 | p14 | 同义归一只救落兜底的句子；原句以过门槛的分数检到**错篇**的不救。本轨自写句里实测三例（基线就有）：「快递一般几天到」答成 PAY-001（意图错，自信答错）、「东西要寄到另外一个地方」答成退换货篇、「说好退我的钱一直没见着」按 LOG-004 转人工（路由对、意图错） | 这类句子的读数不因本轨变化；前一类是答错 | 主会话整合期看留出集读数定：要么批准归一常开（test_cs_understand_t173 的开发集摘要表随之重算），要么在重排里加「命中篇的意图与理解层意图冲突时降分」 |
| 2026-09-25 | p14 | 话术库要补说法就得改 maos/tests/test_cs_eval_p12_t169.py 的增补棘轮（基线指纹、只许三类缺口的类别表），那不在任何实现轨的白名单里 | 以后的泛化轨同样只能走代码侧归一，话术库的同义词停在 p12 | 主会话定：把增补棘轮的类别表开放给后续轨（每条仍要写理由、过片段与编辑距离判据），或维持现状 |
| 2026-09-25 | p14 | 契约类别 2 的「下单后要改收货地址」在 p13 路径没有进查单 / 追问单号（诉求 other，走政策检索、LOG-005 转人工），见 DECISIONS task-t182 | 与契约写的 p13 期望不一致（p12 路径的期望不受影响） | 主会话量 p14 留出集时看；要进查单需改 desk.order_need 对 other 的处理（T174 的决定） |
| 2026-09-25 | p14 | SYNONYM_RULES 与问候词表是按类别手写的正则 / 词表，覆盖面受作者想得到的说法限制；自写句之外的说法（方言、错别字叠加口语）仍可能落兜底 | 留出集读数本轨看不到（盲），可能仍不达预登记门槛 | 主会话整合期量 p12 留出集两条路径与 p14 留出集；再不达标时考虑真模型路径的 few-shot 二次判断 |
| 2026-09-25 | p14 | 理解层的问规则标记新加了「哪个 / 哪种 / 先……还是 / 第一步 / 先后 / 顺序」，带这些字又带诉求词、却没有行动标记的句子不再给诉求（例：「退款退回哪个账户」） | 「哪个订单退款了」这类少见说法在 p13 路径可能不进查单、改走政策检索（方向：少查一次单，转人工或答政策） | 看 p14 留出集里 p13 流程的读数 |
| 2026-09-25 | p14 | 条件威胁的转述句（「我朋友说不退就报警，我觉得没必要」）仍判 complaint 转人工 | 多转一次人工（宁可多转，不漏转） | 主会话看 p14 留出集读数；要排除需区分转述与本人威胁（「你们说不退」本人也会这么说），拿不准暂不排除 |
| 2026-09-25 | p14 | 基线就有的 p13 路径自信答错：「旧手机开不了机了想换个新手机推荐一款」按意图提示答 RET-003（意图 return_exchange，实为购买咨询） | 一句答错；不是 T182 引入，同义归一不参与 | 主会话定是否在意图提示的「坏了 + 换」线索上排除推荐 / 换新；会动 test_cs_understand_t173 的摘要表 |
| 2026-09-25 | p14 | 复核意见修正后，订单异常规则对「多收了我一次运费」「一直停在路上」这类说法可能不再命中（收窄的代价） | 这些说法回到修正前的基线行为（落兜底或走原检索） | 看 p14 留出集读数；要补就按物流对象 / 钱的宾语补，不放回裸「件 / 信息 / 多收」 |
| 2026-09-25 | p14 | 基线 45f724d 就有的自信答错（二轮复核实跑核出）：「退火炉怎么操作」两条路径答 RET-002；「我换个问题，衣服起球怎么办」「退订花呗后多久生效」p13 路径答 RET-003 / PAY-001 | 非退货问题被答成退货 / 退款政策 | 主会话量留出集时一并看检索地板与意图提示；本轨不动 |
| 2026-09-25 | p14 | 二轮收窄的代价：配送范围的地名一支只认目的地问法（地名在前、问词不在分句末的说法不再认），发货时间不再认「什么时候发那个」一类，质量换货不再认「能换个别的颜色吗」 | 这些说法回到基线行为（多半落兜底） | 主会话量 p12 留出集时看召回；需要再放宽就按正向结构加，不加黑名单 |
| 2026-09-25 | p14 | 基线 45f724d 就有的 p12 路径自信答错（本轮自写句实跑核出）：「退款说原路返回但还没回到微信零钱」「退款到现在也没回到原来的卡上」「退的钱一直没回到云闪付」答 PAY-001，「退回的钱没到余额里」答 RET-004 —— 钱没到被检索直接召回成政策篇 | 该查单的句子被答成原路退回的政策 | 主会话量留出集时看检索对「没 / 未 + 到达」的处理；本轨同义归一不参与，不动 |
| 2026-09-25 | p14 | 留出集测试的失败消息（EvalMiss 的 repr）带出了留出句原文，与契约「失败消息只报聚合数与 id」不一致 | 实现者会被动看到留出句，盲写失效 | 主会话收紧 test_cs_eval_p12_holdout.py 的失败消息（只报 id）；本轨白名单外，不动 |
| 2026-09-25 | p14 | T182 二轮复核余项：辱骂词（白痴 / 弱智 / 神经病）在分句末时，指第三方也触发 anger；「这个 / 这种 + 机器人 / 服务」的狗字支会认进商品问题；「找平台 / 上网 / 发朋友圈」的条件威胁在过去时叙述与明说不追究时也判 complaint；「电池需要换吗」类分句末「换」的宾语在前时仍会走质量换货篇 | 前三条是误转人工（安全侧，体验差）；最后一条是答了质量换货的政策篇（政策本身不错，但问的是零件） | p15 若继续理解层：从正则扩词转到带否定 / 指代的结构化判定，不再逐句补 |

## integrate-p14-a1（p14 整合期：A1 合流时留下的事，2026-09-25）
...
## integrate-p14（p14 整合收尾时留下的事，2026-09-25）

| 日期 | Phase | 现象 | 影响 | 建议处理时机 |
|---|---|---|---|---|
| 2026-09-25 | p14 | **证据束没有按 p14 重产**（用户拍板留给 Mac） | 本分支上 verify 第 9 项在重产前判负 | 作者 Mac：`python3 scripts/make_evidence.py` 后单独一个提交 |
| 2026-09-25 | p14 | **两份留出集的预登记门槛都还有没达到的项**：p12 intent 0.8429 / handoff 0.8684；p14 route 0.84 / 措辞 ≈0.82；安全面（编造 0、错状态 0、零自信答错）全部守住 | 客户换说法仍有一部分落兜底或转人工 | 下一期按 integrate-p14-a1 那行的类别开轨，并**另盲写**一份新留出集（p14 这份主会话已看过误判明细） |
| 2026-09-25 | p14 | 理解层的泛化手段是正则扩词，T182 两轮复核都在「收窄误伤 ↔ 扩召回」之间来回 | 继续扩词边际收益低、误伤面难穷举 | 下一期考虑带否定 / 指代的结构化判定，或在真模型可用时让 cs.understand 兜底（行为仍须确定性可测） |
| 2026-09-25 | p14 | MCP 连接器只做了 stdio；ADP 侧的真接入（远程传输、鉴权）没做 | 方案 B 目前只在本机可演示 | 用户拍板 ADP 接入方式后 |

## task-t183（p15 盲写留出集，2026-09-25）

| 日期 | Phase | 现象 | 影响 | 建议处理时机 |
|---|---|---|---|---|
| 2026-09-25 | p15 | p13 契约 §2 第 6a 步没说：追问后的下一轮客户只说「单号不知道在哪」而没重复查物流的词时，算不算仍在要看订单 | 本集把追问用尽前的那一轮写成仍在问物流 / 退款进度（期望 clarify），C3 那一轮才是没线索词的说法；若实现把追问后第二轮判成兜底，会在追问用尽类 case 上连丢两轮 | 整合期主会话看真前台读数时对照；需要的话在契约里补一句「追问进行中的会话，本轮没给单号就沿用上一轮的查单意图」 |
| 2026-09-25 | p15 | 英文政策问题（如英文问七天无理由）的期望 intent 契约未定：p13 不买英文话术库，路由必是兜底，但 intent 是 understand 的意图还是 unknown 没写 | 本集因此一道英文政策题都没出，英文覆盖只在查单 / 触发 / 闲聊 | 下一期若加英文话术库或主会话裁定口径后补题 |
| 2026-09-25 | p15 | 复核 L2-1：本集 161 轮没有一处有意的错别字（同音 / 形近 / 拼音首字母），p12 契约 §1.6 列的「口语化与错别字」在 p15 留出集上不被度量 | 前台对错别字的鲁棒性（含 T186 近邻兜底对错字的召回）这次唯一一次验收读数判不出来 | 下一份盲写留出集把错别字列为覆盖地板（例如 ≥8 轮，同音 / 形近 / 拼音首字母各有），逐轮打 tag，在测试里按 tag 计数钉住 |

## task-t184（判定顺序，2026-09-25）

| 日期 | Phase | 现象 | 影响 | 建议处理时机 |
|---|---|---|---|---|
| 2026-09-25 | p15 | 注入端口时「怎么查物流」照旧追问单号（基线就如此：理解层给了 track）；「我要退货，运费谁出」也追问（明说要办优先，T173 已定） | 问办法的一句被当成要看这一单，多问一句单号，不答错 | 理解层（understand.py，T185 独占）下次改诉求判定时一并看 |
| 2026-09-25 | p15 | desk.py 的 ORDER_VIEW_SCHEMES 按方案编号写死四篇 | 话术库改编号时 C1 的兜网静默失效（本轨测试钉了 PAY-004 与 LOG-005 两头） | 话术库 body 若加「看单 / 办事」字段时改成按字段认 |
| 2026-09-25 | p15 | C3 的 still_on_order（复核 L2-3 / L3-3 后已收窄）仍按说法表认：追问之后的一句若既带「单号 / 忘了 / 找不到」一类说法、又其实在问别的规则，会被再追问一次 | 原先这一行说「不会答错」不准确：会把本该答政策的一轮变成追问或转人工（不说错状态，但少答一句）；收窄后剩的是更窄的这一类 | 看 p15 留出集读数；若仍有，考虑追问后先检索、答得上政策就不接回查单 |
| 2026-09-25 | p15 | 英文的假设 / 政策问题 C4 只挡住了查单，仍回英文兜底（没有英文话术库，p13 契约不买） | 英文政策问题仍落兜底，连续两轮转人工 | 英文话术库立项时 |
| 2026-09-25 | p15 | C1 改道的「订单异常说法」表（_ZH_ORDER_ANOMALY_RE）与中文查物流说法的句尾表都是手写说法表 | 没列到的异常说法检索落到查单篇时照基线转人工（不答错，但不追问单号） | T186 的近邻兜底落地后看是否改用检索分数 / 话术篇字段认 |
| 2026-09-25 | p15 | 否定查单的句子若同时被理解层给了 track 诉求或命中进度线索（「别查快递了，七天无理由怎么算」「no need to track my parcel, do you ship to Canada」），注入端口时仍追问单号 —— 基线 7c19a1d 同样如此，不是本轨引入 | 客户多被问一次单号，不会编造状态 | 理解层（understand.py，T185 的面）与线索表下次改动时一并看否定说法 |
| 2026-09-25 | p15 | C4 的「的话」话题判定、条件在前另起一句的判定都是手写说法（订单名词表、分句标点、另起词表） | 没列到的话题名词（如「那票货的话到哪了」）仍按假设走政策检索，结果同上一轮 | 留出集下一次盲测后再看 |

## task-t185（单号抽取与规范化，2026-09-25）

| 日期 | Phase | 现象 | 影响 | 建议处理时机 |
|---|---|---|---|---|
| 2026-09-25 | p15 | 已落盘库里若有旧口径写入的带前缀绑定行（display_no 形如 #1001、单号：A1001），新口径核验按去前缀后的值查，对不上 | 只影响 p15 之前用带前缀种子写过的库；种子文件本身重新 load_bindings_file 即按新口径写入 | 整合期或部署时：对持久库重跑一次种子加载；必要时加一次性迁移脚本 |
| 2026-09-25 | p15 | evaluate.FixtureVerifier 按夹具 display_no 原样精确比对，不过 normalize_display_no | 夹具里若写带前缀的 display_no，评测会与真 BindingVerifier 结论不一致 | 下一次动 evaluate.py 的轨（白名单外，本轨不改） |
| 2026-09-25 | p15 | extract_order_tail 已提供但前台没接（尾号只是提示） | 转人工卡片上看不到客户给的尾号 | 整合期决定要不要写进卡片（不进槽位、不查单） |
| 2026-09-25 | p15 | 复核 L3-3（基线就有，不是本轨回退）：取件码 / 提货码 / 验证码 / 配送员编号 / 运单号 / 快递单号 / 明写「型号」后面的串仍当订单号抽出（「快递单号 SF1234567890 查不到」「型号 A2894 的充电器有问题」） | 前台会拿这些号去核验（转人工 identity_unverified）或挡住政策判定 | 下一次动单号抽取的轨：给「别的号」字眼表加这些词，注意「快递单号」含「单号」，要确认最近字眼优先时 other 能赢 |
| 2026-09-25 | p15 | 复核 L3-A（基线就有，不是本轨回退）：不带品类名词的光秃秃型号（RTX4090 / XPS9530 / WH1000XM5 / HD6500 / GTX1660）加政策问法，「RTX4090 能七天无理由退吗」型号进 order_no，政策问句掉进 fallback；另外「机型 A2894」「SKU12345」「版本 V12345」也仍当订单号抽出 | 客户问政策拿不到政策回答（安全失败：fallback，不答错）；C8 说非单号的数字不许挡政策判定 | 整合期或管判定顺序的 T184：二选一 —— 前台不让「附近没有单号字眼的 order_no」挡住政策问句（推荐，改判定顺序一处），或抽取在政策问句且无单号字眼时不认字母数字串；给 L3-3 的「别的号」字眼表加型号时一并加机型 / SKU / 版本 |

## task-t186（检索的确定性近邻兜底，2026-09-25）

| 日期 | Phase | 现象 | 影响 | 建议处理时机 |
|---|---|---|---|---|
| 2026-09-25 | p15 | 近邻通道在现有例句上贡献很小：自写 C9 / C10 79 句里词法零命中的 19 句，在零误命中的门槛下只救回 1 句（其余正例的余弦 0.07–0.22，与反例同一档） | C9 / C10 两类的口语说法仍多数落兜底，留出集上的收益预计很小 | 主会话定是否把 maos/tests/test_cs_eval_p12_t169.py 的增补棘轮（GAP_CLASSES_T169）开放给 C9 / C10 两类；开放后另起一轮只补例句（每篇 ≤ 8 条）并在开发集 + 自写例句上重扫阈值 |
| 2026-09-25 | p15 | 词法重排在自写句「东西原封不动的，我能不能退回去」上自信答错（RET-001 该答，给了 PAY-001；两篇共享「退回」） | 零「自信答错」不变量在自写句上有一处例外，与近邻无关 | 下一次动 scripts 重排时处理 RET-001 / PAY-001 的「退回」歧义 |
| 2026-09-25 | p15 | 否定句词法照旧检到查单篇：「我不是要退货，只是想问问怎么保养」「我没有要退款，东西挺好的」→ RET-005、「不用改地址了，地址是对的」→ LOG-005，都转人工 | 安全方向（多转人工），但客户体验差、人工多接一单 | 判定顺序或理解层处理否定时一并收（T184 同类） |
| 2026-09-25 | p15 | 近邻通道每次启用都按本租户全部话术重建 TF-IDF 索引，没有缓存 | 19 篇 × 例句，开销可忽略 | 话术库明显变大时按 (tenant, doc_id, created_at) 缓存索引 |
| 2026-09-25 | p15 | 复核 L2-2：C10 对 p13 路径期望「进查单 / 追问」，而近邻落到带转人工标记的查单篇、原文没单号时，现状是 needs_order_lookup 直接转人工、不追问（与词法命中同一篇时一致） | 目前只潜伏：现有例句与阈值下 p13 路径上近邻只在 GEN-002 上启用；扩例句或降阈值后才会出现 | 整合期：是否先追问单号由 desk.py 的判定顺序（T184）决定，match_scripts 交不出；按 T184 的顺序改 test_handoff_marked_doc_via_similar_on_p13_path_t186 的钉子 |

## integrate-p15（p15 整合收尾时留下的事，2026-09-25）

| 日期 | Phase | 现象 | 影响 | 建议处理时机 |
|---|---|---|---|---|
| 2026-09-25 | p15 | **证据束没有按 p15 重产**（用户拍板留给 Mac） | 本分支上 verify 第 9 项在重产前判负 | 作者 Mac：`python3 scripts/make_evidence.py` 后单独一个提交 |
| 2026-09-25 | p15 | 「零自信答错」只判到意图级：同一意图下答错篇（开票时长 → 退款到账时长）判不出 | 客户拿到一篇不对题的政策 | 下一期：评测与留出集测试加「route=answer 时 cite 必须对」的篇级不变量 |
| 2026-09-25 | p15 | p15 留出集剩余误判类别（主会话归纳）：口语寒暄 / 致谢与「周末有人值班吗」一类客服时间问法仍落兜底并连带 repeated_fallback；运费凑单、开票形式、换货要不要拍照等细分政策问法召回不到；地址改错（门牌号改写）在注入端口时直接转人工而不是追问单号；中转站「趴了」多天一类进度说法；单号前带 No. 且无空格时没进查单；「帮我查呗，我翻不到单号」在追问用尽后落兜底 | 措辞 0.89 未达 1.0；其余四项已达 | 若再开一期：T186 的例句扩充被 test_cs_eval_p12_t169.py 的增补棘轮（GAP_CLASSES_T169）挡住，要先由主会话决定是否对 C9 / C10 开放；并须另盲写新留出集（错别字列为覆盖地板，见 task-t183） |
| 2026-09-25 | p15 | T184 余项 L2-2 / L3-A（见 integrate-p15 DECISIONS） | 政策问题被推向追问 / 查单 | 同上 |

## task-t187（p16 盲写留出集，2026-09-26）

| 日期 | Phase | 现象 | 影响 | 建议处理时机 |
|---|---|---|---|---|
| 2026-09-26 | p16 | 契约未钉「办具体订单的退货 / 退款」轮的意图（refund_payment 还是 return_exchange），也未钉 p13 路径「带单号问退款进度」是否因 request=refund 进预检 | p16 留出集按 RET-005 取 return_exchange，且不出后一种题；若前台取法不同，intent 读数会有与泛化无关的噪声 | 整合期量 p16 留出集时若 intent 误判集中在退款桥轮，由主会话裁定口径并写进下期契约 |
| 2026-09-26 | p16 | 留出集按轮的类别 / 错别字标注（gaps / typos）只是留出集文件的约定，evaluate.py 与 cs_eval.py 不认 | 分类别、分错别字种类的读数现在出不来，只能看总分 | 若要按类别出读数：下期由评测轨把这两个可选键收进 EvalCase 并在报告里分组 |

## task-t188（例句扩充与近邻阈值重扫，2026-09-26）

| 日期 | Phase | 现象 | 影响 | 建议处理时机 |
|---|---|---|---|---|
| 2026-09-26 | p16 | 自写句上词法已有的七句答错 / 转错篇（「多长时间」把发货问法拉到退款时长篇、「指定 / 别的快递」被查单篇拉走、「快递费」被退货运费篇拉走、「申请 + 退」「退款钱呢」被流程 / 运费 / 办退款篇拉走） | p16 起「零自信答错」是篇级：这类句子一旦出现在评测集里就计入 | 改词法重排（maos/domain/cs/scripts.py，不归本轨）；本轨的测试以封闭表钉住，修好那天表里删句子 |
| 2026-09-26 | p16 | 否定句与第三人称只在近邻通道硬弃权，词法照答：「不用退了，我自己留着」→ RET-004、「我不需要发票」→ PAY-005、「他们家包邮吗」→ LOG-003；另「能开个会员吗」→ PAY-005（开 + 个） | 不该答的被答成政策话术（意图级不算错、篇级也无「对的篇」，但客户拿到不对题的回复） | 与 BACKLOG task-t186 否定句那条合并处理：词法层加同样的否定 / 转述线索 |
| 2026-09-26 | p16 | 「能用银行卡付吗」「可以用apple pay吗」一类支付方式问法：增补后会把 T182 列为反例的「用邮政储蓄卡付款可以吗」「狗狗app怎么下载」答成 PAY-002 | 前一句其实问的就是支付方式（答 PAY-002 是对的），T182 的反例表把它当「邮政 = 快递」的误伤来防；后一句是字符巧合（app） | 主会话裁定 T182 那句反例的期望；支付方式的例句扩充等裁定后再补 |
| 2026-09-26 | p16 | 词法二元组重排对例句没有「须含本篇核心词」的约束：例句里的一个生僻词或一段口语填充（卡着不动了、要改一下、显示完成了、没付款的…）就够短句过 MIN_SCRIPT_SCORE 0.25。复核轮为此撤掉 37 条增补；剩下的增补仍让 14 句无关探针新转人工（封闭表 NEW_HANDOFFS_T188） | 例句扩充的收益被这条卡住：增补越多，无关句被拉进来的越多；本轨独立量得的增益只剩 +1 / 103 | 改 maos/domain/cs/scripts.py（不归本轨）：只凭例句命中时要求与该篇场景 / 同义词共享至少一个核心二元组，或给例句命中单独的更高门槛；改完再放开例句扩充 |
| 2026-09-26 | p16 | 别的平台 / 别家的问题被运费篇拉走：闲鱼卖东西要运费吗 → LOG-003；另 我不想开发票 → PAY-005（否定句）、不用了谢谢，我不买了 → GEN-002（复核 L3-2 的实测，增补前后相同） | 不是本轨回归；前一句是新类别（问别的平台），后两句与本节第二行否定句那条同源 | 与本节第二行、BACKLOG task-t186 否定句那条一起处理；「别的平台」需要词法层的平台名线索 |

## task-t189（篇级零自信答错，2026-09-26）

| 日期 | Phase | 现象 | 影响 | 建议处理时机 |
|---|---|---|---|---|
| 2026-09-26 | p16 | 本轨按盲约束没看留出集及其测试：p12 / p14 / p15 / p16 留出集的测试还没钉篇级 confident_wrong（读数未知） | 留出集上的篇级自信答错只在 cs_eval 输出里可见，没有棘轮 | 整合期主会话量一次，按读数钉地板（只许降不许升） |

## integrate-p16（p16 整合收尾时留下的事，2026-09-26）

| 日期 | Phase | 现象 | 影响 | 建议处理时机 |
|---|---|---|---|---|
| 2026-09-26 | p16 | **证据束没有按 p16 重产**（用户拍板留给 Mac） | 本分支上 verify 第 9 项在重产前判负 | 作者 Mac：`python3 scripts/make_evidence.py` 后单独一个提交 |
| 2026-09-26 | p16 | 「状态问句 + 退款诉求」同一句里（还没发货吧，我想退款）前台只答了状态、没走退款桥 | 客户要办的事没办，得再说一遍 | 下一期判定顺序：同句里有退款 / 退货诉求时诉求优先于状态问 |
| 2026-09-26 | p16 | 「邮费谁出」不带「退货」时落到退货运费篇 | 答非所问（两篇都讲运费，客户能看懂，但不对题） | 下一期：运费篇与退货运费篇的分界说法 |
| 2026-09-26 | p16 | 查单作答那一轮的意图标签沿用上一轮（措辞与状态都对） | 只影响统计口径，不影响客户 | 下一期理解层：本轮带单号且问进度时意图以本轮为准 |
| 2026-09-26 | p16 | 错别字轮 23 轮错 5 | 同音 / 形近 / 拼音缩写仍有一部分落兜底 | 若再开一期，另盲写新留出集（p16 这份已读过） |
