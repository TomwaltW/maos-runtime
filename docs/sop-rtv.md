...
2. **`exchange` 与 `replacement` 只做到裁定为止。** 契约 C-R4 的六个 skill 里
   ...
   的流程，当前是**空的**。
3. **场景演示与 skill 测试用的都是契约级 stub。** 演示场景按契约的 skill 名注册
   stub skill、按契约的返回值形状造 stub 工具、表结构自己按 C-R1 建；skill 层的
   测试同样靠注入 stub 工具跑 —— 因为四轨并行时上游代码互相不在对方基线里。
   整合期才换成真的。哪些是 stub、整合期换成什么，写在场景文件头。
   「整合期把 stub 换成真的、编排逻辑一行不用改」这句话本身**也还没有被验过**。
...
8. **「复用」在表这一层是真的，在代码那一层是有意的重复。** 五张表确实一张不重建
   （§6.1 的命令可核），但金额精度、业务对象引用这些口径，RTV 域是**照抄** `ap`
   域的几十行、不是 import 它 —— 跨域 import 会让「换域只换 Skill / ToolPort /
   业务对象」当场破产。这份重复是有意的，代价是 `ap` 域那几十行改了、RTV 这边
   不会自动跟着改。真要去重，落点是一层与域无关的公共基础设施 —— 那条提议记在
   [`BACKLOG.md`](BACKLOG.md)（原文明写「上第三个域之前再决定」），至今没做，
   而 RTV 已经是第五个域了。
