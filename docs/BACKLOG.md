...
基线 `c1049c2`。以下是本轮**按铁律 4 不当场改**的账。
...
| 2026-08-29 | P7 | **`docs/clone-smoke-report.md` 的读数本轮没刷**（`521 passed` / `571 passed` / `4/74` / `0/33` / 逐步耗时） | 该文件是**某次冒烟的实测记录**，带明确的基线 sha 标注与时效声明，里面每个数字都是「那一次跑出来的事实」。照当前值改写它，等于伪造那次冒烟的结果 —— 比留着旧数字坏得多 | **不建议刷**，建议改判据：把它明确定性成「历史快照，只增不改」，需要新读数就**另跑一次冒烟、另起一节**，而不是回头改旧节。若下一轮决定重跑，注意 §3 那张「七行实跑读数还准不准」表里的 `74/74`/`33/33`/`18/18`/`9/9` 一并会变 |
