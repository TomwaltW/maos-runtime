...
> `python3 run.py` 仍 exit 0、1361 条测试仍绿，坏的只有 4 个房间测试文件（206 条）。<!-- metric:frozen A1 实验当次的读数，权威记录（含逐条命令与原始输出）在 docs/defense-brief.md 的 ## A1；要刷得连那份一起重跑重写，单刷此处会与出处对不上 -->
