...
def html_block(text: str) -> str:
    """把多行正文转成 Matrix 的 ``formatted_body``：**转义 + 换行 + 缩进**，三样都要。

    换行那一样最容易漏，而漏了不报错：Matrix 的 ``body`` 是纯文本，``\n`` 在里面
    是换行；但 ``formatted_body`` 是 HTML，``\n`` 在浏览器里只是空白，会被折叠掉。
    症状是一份排好的逐条清单在 Element 里流成一大段，`· 第 10 行 …· 第 18 行 …`
    首尾相接（2026-09-10 的房间实测）—— 而发消息那一侧一切正常，日志里也看不出来。

    不用 ``<pre>``：它保住缩进的代价是浏览器一律不折行，一段没有换行的长正文
    （闲聊回话正是这样）会摊成一条横向长条，Element 里左右两头都被裁掉
    （2026-09-06 实测）。``<br/>`` 负责换行，行首 ``&nbsp;`` 负责缩进。
    """
    lines = []
