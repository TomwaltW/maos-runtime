...
| 2 | **Member / Worker** | 可插拔 Agent 池：role → Agent 类，投放一个文件即注册；Worker 收到 `TaskAssignment` 按 role 取执行者 | `maos/agents/base.py:101`（`AGENT_POOL`）<br>`maos/agents/base.py:104`（`@register`）<br>`maos/runtime/worker.py:34`（一行构造全池） | ✅ 已跑通（10 个角色，见 [`agent-identity.md`](agent-identity.md)） |
