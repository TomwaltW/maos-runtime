"""理赔域四个 Agent 的共用件 —— artifact 形状与 extras 口径。
...
def extras_of(agent: Any, ctx: Any) -> dict:
    ...
    而 invoker 自己生成的那个到不了 skill 里 ——
    详见 `maos/skills/builtin/claim/_common.py` 的模块 docstring 第 2 条。
...
def artifact(kind: str, content: dict, *, summary: str) -> dict:
    ...
    缺了就是 rework，而症状会显示成「理赔流程走不完」，离原因极远。

    `self_check` 恒 pass 不是走过场：理赔域产物没有 build/lint 这回事，本域真正的
    验收判据是条款版本、赔款核算与到账回执 —— 那些由本域的守卫与断言把守。
    """
    body = dict(content)
