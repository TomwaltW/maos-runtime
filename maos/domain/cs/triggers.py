...
2. **只加不减**：p12 漏掉的中文骂法（:data:`ADDED_PATTERNS`：操你妈 / 他娘的 / 尼玛 / tmd……；
   p14 T182 补了辱骂客服质量的说法「破客服 / 气疯 / 客服是摆设」与条件威胁「不 X，我就差评 /
   报警 / 找媒体 / 上网说」）与英文触发词（:data:`EN_PATTERNS`）。它们只会让一句话多判出原因、
   判出更高优先级的原因，不会让任何一句少判。
3. **唯一的例外：名词性良性复合词**（:data:`BENIGN_NOUNS`，一张封闭的名词表：商品、地点、
   日常名词 —— 「空气炸锅」里的「气炸」、「去死皮膏」里的「去死」、「人工草坪」里的「人工」）。
   名词按子串认，所以只收在触发词外面至少多出两个字、且多出的那截不是常紧挨触发词的另一个词的；
   「垃圾 / 曝光 / 补偿」本身就能带名词（什么垃圾袋子、曝光度假村、补偿器材），它们打头的一条不收
   （「隐私贴」会认进「隐私贴到网上」，也不收；见 :data:`BENIGN_NOUNS` 的注释）。
   一句话里触发词的**每一处**匹配都完全落在某个表内名词里、且句中**没有任何别的触发词**
   （含英文、感叹号、12315）时，这句不触发；只要还有一处别的触发词，表内名词里的那几处
   照常算，优先级不因复合词降低（「温度补偿坏了，转人工」仍是 compensation，同 p12）。
...
# 第 2 层：只加不减（p13 T173）
# ---------------------------------------------------------------------------
#: 分句末：后面紧跟语气词、标点或句末（p14 T182 的「有病 / 摆设 / 死人」用，复核 L3-4）。
_CLAUSE_END = r"(?=$|[吧啊呀吗么嘛呢啦了哦,.!?~，。！？])"
...
ADDED_PATTERNS: dict[str, tuple[str, ...]] = {
    HANDOFF_ANGER: (
        ...
        r"去你的(?=$|[吧啊呀,.!?~，。！？])",
        # ---- p14 T182：辱骂客服质量（p12 只认「什么破 / 破店」，「破客服」「气疯」落了兜底）----
        # 「破 / 烂 / 狗 / 鬼 + 对象」：对象限客服、机器人、服务、平台、系统、店铺、公司、态度
        # （「破东西 / 破包装」多半在说商品本身，不收）
        # 「狗 + 对象」只认冲着对方说的（你们这狗客服 / 什么狗服务 / 真狗态度）：「遛狗服务 / 宠物狗服务 /
        # 训狗服务 / 狗狗app」是在问商品（复核 L3-4、二轮 L2-4 / L3-3：正向结构，不再列前一个字的黑名单）
        r"(?:破|烂|鬼|渣)(?:客服|机器人|系统|平台|服务|店铺|公司|态度|app|软件|回复)",
        r"(?:你们?|你们这|这|这个|这种|什么|啥|真|好)狗(?:客服|机器人|系统|平台|服务|店铺|公司|态度|app|软件|回复)",
        # 气疯 / 气晕 / 气哭 / 气炸了 / 气得要死（p12 的「气死」「气炸」照旧在地板层）
        r"气(?:疯|晕|哭|傻|出病)", r"气得(?:我|人)?(?:要死|半死|要命|不行|发抖|吐血|肝疼|睡不着)",
        # 骂客服蠢 / 没用：白痴、弱智、蠢货、废物（不含「废物利用 / 回收 / 处理」）、人渣、屁用、狗屁
        # 「白痴 / 弱智 / 神经病」要冲着对方说（你是白痴吗 / 客服真是弱智 / 白痴客服），或整个分句就落在
        # 这个词上（「真是白痴！」）：「神经病学的书」「弱智吧的段子」「写得白痴都能看懂」「神经病医院」
        # 不是骂人（二轮复核 L3-3）
        r"(?:你们?|客服|机器人)(?:是不是|真是|就是|简直是|简直|是|真)?(?:个|一个)?(?:白痴|弱智|神经病)"
        + _CLAUSE_END,
        r"(?:白痴|弱智|神经病)(?=$|[啊呀吗么嘛呢啦哦,.!?~，。！？])",
        r"(?:白痴|弱智|神经病)(?:客服|机器人|系统|平台|服务|店铺|公司|app|软件)",
        r"蠢(?:货|猪|死|蛋)", r"废物(?!利用|回收|处理|再)", "人渣", "屁用", "狗屁",
        # 「有病 / 摆设 / 死人」只认分句末（后面紧跟语气词、标点或句末）：「你们有病号服吗」「你有病历本
        # 卖吗」「你们摆设的那个花瓶」「死人头牌」是在问商品（复核 L3-4）
        r"(?:你们?|客服)(?:是不是|是)?有病" + _CLAUSE_END,
        r"(?:客服|你们?)(?:就是|是)?(?:个|一个)?摆设" + _CLAUSE_END,
        r"(?:客服|你们?)(?:是)?死人" + _CLAUSE_END, "听不懂人话", "不说人话",
    ),
    # ---- p14 T182：条件威胁（BACKLOG integrate-p13）——「不 X，我就 <真后果动作>」----
    # 只认**真后果动作**里 p12 词表没有的那几样（投诉 / 曝光 / 12315 / 消协 / 起诉 / 举报本来就认）：
    # 差评、报警、找媒体 / 记者 / 平台 / 律师、发微博 / 抖音 / 小红书 / 朋友圈、上网说 / 挂网上、黑猫、仲裁。
    # 条件必须是**对方不办事**（复核 L2-1 / L3-3、二轮 L3-2）：「不 / 没 / 再不 / 还不」**紧跟处理类动词**
    # （退 / 处理 / 解决 / 发 / 回复 / 赔 / 换 / 补 / 给 …）再「… 就」（「要是还不发货我就…」也在这一支里），
    # 或「否则 / 不然」。光有「如果 / 要是 / 万一」不够：「如果质量好我就发小红书推荐」「万一我忘了我就找平台
    # 客服问」是好话或求助。「不好意思 / 不错 / 不到一周 / 没想到」不是条件。
    # 后果一侧：「就」与动作之间不许有否定或「可以」（「就不会给差评」「就不用找平台了」「就可以给差评吗」
    # 是表态或问规则）；动作之后同一分句里有「夸 / 好评 / 推荐 / 安利 / 分享 / 宣传 / 晒 / 吗 / 么 / 嘛 /
    # 能删」的不算（夸奖、问句）。
    # 「我不会给差评的」「不退款我就自己留着」「不退的话我会很失望」都不算（纯发泄、拿主意、表态）。
    HANDOFF_COMPLAINT: (
        r"(?:(?:不|没|再不|还不)(?:给我?)?(?:退|处理|解决|发|回复|答复|回|赔|换|补|管|理|改|寄|给)"
        r"[^。.!?！？]{0,10}?就|(?:再|还|继续)拖[^。.!?！？]{0,10}?就|否则|(?<!要)不然)"
        r"(?:(?!不|没|别|甭|可以)[^。.!?！？]){0,8}?"
        r"(?:中?差评|报警|找(?:媒体|记者|电视台|平台|律师|工商)|发(?:微博|抖音|小红书|朋友圈|帖子?)"
        r"|(?:上|到|去|在|发)网上?(?:说|发|曝|讲|挂|宣传|评)|挂(?:到|在)?网上|黑猫|仲裁)"
        r"(?![^,，。.!?！？;；]{0,6}?(?:夸|好评|推荐|安利|分享|宣传|晒|吗|么|嘛|能删))",
    ),
}
...
#:
#: 名词是按**子串**认的（不看上下文，裁定 R1 不许用上下文正则），所以还有三条一般的收词规矩
#: （复核 L2-1 / L3-2：「隐私贴」认进了「隐私贴到网上」、「垃圾箱」认进了「垃圾箱子」）：
#:
#: * **名词在触发词外面至少多出两个字**（左右合计）。只多一个字的（「垃圾袋 / 垃圾桶 / 气炸锅 /
#:   隐私贴 / 身份证夹 / 真人发 / 人工费 / 滚刷 / 滚石 / 曝光值 / 补偿器 / 人工湖 / 摇滚」……），那一个字
#:   常常只是下一个词的词头紧跟在触发词后面（贴到 / 夹在 / 发消息 / 费劲 / 刷单 / 石头 / 值得 / 器材 /
#:   湖北 / 箱子 / 篮球 / 夹克 / 桶装水），子串一样、意思完全两样，拿不准就不收。
#: * **触发词本身就能直接带一个名词的，不收「触发词 + 名词」**：「垃圾 + 名词」就是骂（什么垃圾袋子 /
#:   垃圾分类页面 / 垃圾处理器）、「曝光 + 名词」就是要曝光它（曝光度假村 / 曝光时间线）、「补偿 + 名词」
#:   就是要赔（补偿导线的钱 / 补偿器材）—— 这三个词打头的复合词一条都不收（名词在它**前面**的
#:   「背光补偿」照收：前面是主语的位置，不是宾语）。
#: * **多出来的那截本身不能是常紧挨着触发词出现的另一个词**：「真人cs」的「cs」（客服）、「自动曝光」的
#:   「自动」（自动曝光了我的信息）、「长曝光」的「长」（家长曝光 / 店长曝光）都不收。
#:
#: 三条都只会让名词表变小、让句子多转人工，不会让任何一句少判（测试 ``*_t173`` 逐条钉住）。
#: 裁定举的例子里「气炸锅 / 垃圾袋 / 垃圾桶 / 垃圾分类」因此不收（同「死海」），只剩「空气炸锅」。
BENIGN_NOUNS: dict[str, tuple[str, ...]] = {
    "商品": (
        # 气炸：厨房电器（「空气炸锅」左右各多一个字，合计两个）
        "空气炸锅", "气炸烤箱", "空气炸烤箱", "气炸一体机",
        # 去死：去角质的护理品
        "去死皮膏", "去死皮霜", "去死皮凝胶", "去死皮刀",
        # 气死：老式马灯
        "气死风灯",
        # 滚：清洁用品（滚筒 / 滚轮 / 滚珠 p12 本来就不认）
        "粘毛滚",
        # 人工 / 真人：人造材料、医用品、手办与模特
        "人工草坪", "人工草皮", "人工钻石", "人工宝石", "人工水晶", "人工晶体", "人工泪液", "人工耳蜗",
        "人工鱼饵", "人工皮革", "真人手办", "真人娃娃", "真人模特",
        # 隐私 / 身份证：遮挡玻璃与门帘、证件卡套
        "隐私玻璃", "隐私门帘", "身份证卡套",
    ),
    "地点": (
...
#: 英文的名词性复合词（写成空格分词的小写形态），只遮英文触发词这一层。单数的「trash can /
#: garbage can」不收：「this is trash can you refund it」里的「can」是情态动词（同中文那条规矩：
#: 多出来的那截不能是常紧挨触发词的另一个词）。
EN_BENIGN_NOUNS: tuple[str, ...] = (
    "human hair", "trash cans", "trash bag", "trash bags", "trash bin", "trash bins",
    "trash compactor", "trash compactors", "trash grabber", "trash grabbers", "trash picker",
    "trash pickers", "trash liner", "trash liners",
    "garbage bag", "garbage bags", "garbage cans", "garbage bin", "garbage bins",
