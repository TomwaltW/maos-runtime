...
>
> <!-- metric:frozen-begin 上面这条时效声明的机器版：本报告全文的读数都是历史执行记录，各自钉在该节声明的基线 sha 上，改写它们等于伪造那几次冒烟（见本报告 §5）；现行值在 docs/expected-metrics.json -->
>
...
<!-- metric:frozen-end 全文史料到此为止；本行之后再写读数，就要按现行值标 metric:<键名> -->
