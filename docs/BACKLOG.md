...
| 2026-09-10 | p10 | `maos/tests/test_expected_metrics.py` 在本轨的工作区上是红的：新增 78 条测试而 `docs/expected-metrics.json` 按契约 §A 不许各轨去刷 | 本波五轨都会红这一条，**是预期内的**（同 `verify.py` 的 `provenance 4/5`）。红着的时候它守不住别的东西 | 整合期统一刷：收集条数 3261 -> 3339（本轨贡献 78 条 = 44 条无库可跑的翻译器测试 + 34 条 PG 门禁测试），`pytest_passed_nopg` 3220 -> 3264，`pytest_skipped_nopg` 41 -> 75；`docs/submission-checklist.md:24` 的锚点行跟着刷。**五轨合完之后按实跑末行重新取数**，不要拿这里的数字直接填 —— 这只是本轨单独的增量 |
