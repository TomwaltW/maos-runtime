# Demo 分镜（总长 4 分 40 秒 / 280s，已含 Y-4）
...
```bash
cd <repo>
python3 -m pytest maos/tests -q          # 应 703 passed
...
## 03:01 — 03:39　补偿与收口：从未进入 settled
...
| 3 | **审批由谁驱动** | **场景脚本自动驱动，行号复核后未变。** `hq.decide()` 实测仍在 `maos/flows/scenario_7.py:318`（finance，`approved=True`）与 `:351`（payment，`approved=False`）；`decide` 本体在 `maos/runtime/gate.py:586`。不是真人在房间里发命令 | 01:35 与 03:01 两镜 | 念词用「主管审批」没问题（那确实是 HITL 停点），但**别说「我现在在聊天室里点一下」** —— 除非第 1 项已解决并真的现场发命令 |
