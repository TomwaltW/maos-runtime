...
结论只有四种。下面四张卡的**数据**来自本机 2026-09-03 实跑
（`python3 scripts/room_team_smoke.py --evidence scenarios/custom/evidence`，
五岗事实卡逐字真实）；`headline` 的措辞由跨轨契约 §2.2 逐字规定，
以合议引擎并入后的实现为准。
...
建议批复 · 核准预演 9600.00 · 请 supervisor 拍板
  · 证据核验岗：证据核验结论：complete
  · 风险反欺诈岗：风险档位：low（评分 10）
...
建议升级审批 · 风险 high · 请 finance_manager 复核
  ...
  ⚠ 风险：申报金额 92000.00 高于订单实付 88000.00（比值 1.05），核算会按实付封顶
...
🔴 合议引擎（`maos.roundtable.verdict`）并入之前，上面四张卡在房间里**看不到**，
五岗发言与命令面照常 —— 那是预期，不是回归。冒烟脚本这时会打一行
「合议引擎未装载」。
