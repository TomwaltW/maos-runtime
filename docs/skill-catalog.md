...
实现：`CodeRepoPatchSkill` @ `maos/skills/builtin/code_repo_patch.py:154`
...
| 要素 | 含义 | 值 |
| :-- | :-- | :-- |
| `purpose` | ① 用途 | 客服前台一轮的理解：判语种、抽槽位（订单号 / 商品 / 问题 / 诉求 / 情绪，跨轮合并）、判意图（触发词 → 本轮诉求 → 关键词词表；判不出且注入真模型才问模型） |
...
| `failure_policy` | ⑥ 失败策略 | escalate |
| `max_retries` | ⑥ 失败策略 · 重试上限 | 0 |
| `security_boundary` | ⑦ 安全边界 | 只读、不调任何工具、不写任何业务表、不落事件：槽位只从词表与正则来（不自由抽取，客户的住址、手机号进不了槽位），手机号（含连字符、空格与 +86 写法）、座机与 400 / 800 热线不当单号，紧跟在卡号 / QQ / 身份证 / 电话字眼后面的号、没有单号字眼的身份证号与银行卡号形态也不当单号；只在规则判不出意图且注入真模型时调一次模型，唯一的写是那一行模型账；模型出错记一行失败、按规则结果返回，不重试（重试会重复记账）。不碰审批 / 放款 / 补偿 / 工单 / 查单任何一条路。 |
...
### refund.reason_classify @ 1.0.0

实现：`RefundReasonClassifySkill` @ `maos/skills/builtin/refund/reason_classify.py:141`

| 要素 | 含义 | 值 |
| :-- | :-- | :-- |
| `purpose` | ① 用途 | 把客户自由文本的退款诉求归一成 reason_code，附置信度与理由 |
| `input_schema` | ② 输入 | `text`: str（客户/客服写的原文；空串按 unknown 处理，不抛）<br>`candidates`: list[dict{code,label,hint}]?（不给就用内置三个 code） |
| `output_schema` | ③ 输出 | `reason_code`: str（候选 code 之一，或 unknown）<br>`confidence`: float（0.0~1.0）<br>`why`: str（一句话中文理由，给人看）<br>`source`: str（lexicon \| model \| fallback）<br>`raw_text`: str（原文回显）<br>`invocation_id`: str（本次调用的 actor 锚点） |
| `preconditions` | ④ 前置条件 | `text` |
| `depends_tools` | ⑤ 依赖工具 | （空） |
| `failure_policy` | ⑥ 失败策略 | retry |
| `max_retries` | ⑥ 失败策略 · 重试上限 | 1 |
| `security_boundary` | ⑦ 安全边界 | 只读入参，不写业务表、不碰网关；词表命中不调模型 |
| `reuse_note` | ⑧ 复用说明 | 申请受理岗认不出诉求类型时的统一入口；判定仍归规则，本 skill 只产标注 |
| `owner_roles` | ⑨ 归属角色 | `refund_intake` |
