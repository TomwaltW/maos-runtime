...
| 2026-08-27 | P0 | compensation 的 mode 是否锁死取值域 | validate_artifact 只校验字段名、嵌套层级与类型，不限制 mode 的取值 | C-5 的反例针对的是「字段名与嵌套层级各凭记忆」，冻的是形状；把 mode 硬锁成单值 reverse，会在 Task-D 需要别的补偿模式时变成一条假红 |
