...
#: 笑声用字：整句只由它们组成（两个字以上）时是笑，不是招呼。
_LAUGH_CHARS = frozenset("嘿哈嘻呵")
...
def greeting_only(text: str) -> bool:
    ...
    if not s or len(s) > GREETING_MAX_CHARS:
        return False
    if len(s) >= 2 and set(s) <= _LAUGH_CHARS:
        # 「嘿嘿嘿 / 嘿嘿哈」是笑声，不是招呼（同「哈哈哈」；复核 L2-4）
        return False
    i, greeted = 0, False
    while i < len(s):
...
#: 分句内的「若干个字」（不跨任何标点）。
_C = r"[^,，。.!！?？;；]"
#: 物流对象（订单异常的主语）：不收裸「件」（配件 / 软件 / 文件 / 这件衣服）与裸「信息」（复核 L2-4 / L3-2）。
_PARCEL = r"(?:物流|快递|包裹|单号|轨迹|单子|(?:我|我们|我家)的件|快递件|包裹件)"
#: 「没收到 / 找不到」之后紧跟的：分句末、语气词，或说的就是包裹本身（「签收了没看到说明书」不算）。
_MISSING_END = r"(?=$|[,，。.!！?？;；呀啊呢吧啦哦]|快递|包裹|东西|货|件|人)"
#: 发货的动词：只认「发货 / 发出 / 寄出 / 出库 / 出货」，或分句末的裸「发 / 寄」（「能发吗」）。
#: 「发」后面跟别的宾语（发工资 / 发新品 / 发通知 / 发布）都不是发货（复核 L2-2、二轮 L3-1：正向结构，不再列黑名单）。
_SHIP_VERB = (r"发(?:货|出|走|快递)|寄(?:出|走)|出库|出货"
              r"|(?:发|寄)(?=$|[吗么嘛不呢吧啊呀了,，。.!！?？;；])")
#: 假设 / 泛问的框架（「万一 / 如果 / 要是 … 怎么办」「是不是要」「会不会」）：问的是规则，不是自己那一单
#: 已经出了事（二轮复核 L3-4）。订单异常的规则一律带上。
_HYPOTHETICAL = r"万一|如果|要是|假如|若是|的话|是不是要|要不要|会不会|还是"
#: 退货这个**动作**：能退 / 可以退 / 退货 / 退掉 / 分句末的「退」（「退吗」）。裸「退」会认进退烧、退订、退出。
_RETURN_ACT = (r"(?:能|可以|可不可以|能不能|可|想|要|申请|给我?)退|退(?:货|掉|换)"
               r"|退(?=$|[吗么嘛不呢吧啊呀了,，。.!！?？;；])")
#: 配送范围里的地名。
_REGION = (r"(?:偏远|乡下|农村|乡镇|村里|山区|国外|海外|境外|港澳台?|香港|澳门|台湾|西藏|新疆|内蒙古?"
           r"|青海|宁夏|甘肃|海南|岛上)")
...
SYNONYM_RULES: tuple[tuple[str, str, str, str, str], ...] = (
    ...
    # 主语只认物流对象（复核 L3-2：裸「件 / 信息」认进了配件、软件、会员信息、上新信息）；
    # 「一直在」只跟物流地点（「我一直在路上 / 在仓库上班」不是包裹）；天数那一支要句里先有物流对象。
    ("stalled", rf"(?:卡在|停在|滞留在?|压在|躺在|困在|堵在){_C}{{0,8}}?"
                r"(?:中转|转运|分拣|集散|网点|仓库?|站点|海关|路上|物流中心|快递点|营业部)"
                rf"|一直在{_C}{{0,4}}?(?:中转|转运|分拣|集散|网点|站点|海关|物流中心|快递点|营业部)"
                rf"|(?:中转|转运|分拣|集散|网点|站点|海关|物流中心|快递点|营业部){_C}{{0,6}}?"
                r"(?:放|停|卡|搁|压|躺|待|呆|滞留)了?(?:[0-9一二三四五六七八九十两好几多]+天|一周|一个星期|好久|很久)"
                rf"|{_PARCEL}{_C}{{0,8}}?(?:不动|不走(?!样)|不更新|没更新|没有更新|没变化"
                r"|没变|没动静|没动|停了|停住|卡住|卡了|没反应|没进展|一动不动)"
                rf"|{_PARCEL}{_W}{{0,10}}?(?:好几天|几天|多天|一周|一个星期|一个礼拜|[0-9一二三四五六七八九十两]+天)"
                rf"{_C}{{0,6}}?(?:不动|不走(?!样)|不更新|没更新|没有更新|没变化|没动静|没动|停着|卡着|一动不动)",
     "物流没更新", "", r"多久|多长时间|一般|通常|" + _HYPOTHETICAL),
    ...
    ("signed_missing", r"(?:签收|已签|签了|送达|妥投|派送成功|投递成功|放(?:在|到)?了?(?:驿站|快递柜|丰巢|菜鸟"
     ...
     "显示签收没收到", "", _HYPOTHETICAL),
    ("lost", rf"(?:{_PARCEL}|东西|货){_C}{{0,4}}?(?:丢了|弄丢|寄丢|丢失|不见了|找不到了)",
     "包裹丢了", "", _HYPOTHETICAL),
    # 包装那一支只认**已经坏了**的说法（破了 / 压扁 / 湿了 / 凹了 / 瘪了 / 碎了）：「盒子扁扁的是设计吗」
    # 「凹进去的那种有吗」「是瘪的还是鼓的」是在问商品（复核 L3-2），另排除「那种 / 设计 / 有吗」一类询问。
    ("damaged", rf"(?:箱子|盒子|纸箱|外箱|外包装|包装盒?|快递盒|快递袋|袋子|包裹){_C}{{0,4}}?"
                r"(?:破了|破掉|破损|烂了|扁了|压扁|压坏|压烂|湿了|湿透|裂开|裂了|变形了|凹了|凹进去了|瘪了|碎了)"
                r"|(?:拆开|打开|开箱|收到|到手|拿到|取出来|寄过来|寄来|送过来|送来|送到|寄到)"
                rf"{_W}{{0,10}}?(?:碎了|碎成|摔碎|摔坏|摔裂|压碎|压坏"
                r"|压扁|压变形|破了|烂了|裂了|破损|漏了|洒了|撒了|碎的|破的|烂的|裂的)",
     "包裹破损", "", r"换|质量|用了|用就|一用|用过|穿|那种|设计|款式|有没有|有吗|是不是|" + _HYPOTHETICAL),
    ("refund_missing", r"(?:同意|答应|说好|说了|已经|显示|审核通过|通过了?)"
     ...
     "退款还没到账", "", _HYPOTHETICAL),
    # 已然体才算（扣了 / 被扣 / 付了）：「是不是要扣两次」「要付两次吗」是在问规则（二轮复核 L3-4）
    ("double_charge", r"(?:扣了我?|扣我|被扣了?)(?:两|2|二|俩|多|好几|三)(?:次|笔|回|遍|下|份|倍)"
                      # 「多收」要带钱的宾语（「多收藏几个」「多收了一件」「是不是多收一次运费」不算，复核 L2-4 / L3-2）
                      r"|(?:多|重复|重)扣(?:了)?(?:我)?(?:钱|款|费|一次|一笔)?"
                      r"|(?:多|重复)收(?:了)?(?:我)?(?:的)?(?:钱|款|一笔|[0-9一两二三四五六七八九十几]+(?:块|元))"
                      r"|(?:付|支付|交)了(?:两|2|二|俩)(?:次|笔|回|遍)",
     "重复扣款", "", _HYPOTHETICAL),
    ("paid_unpaid", rf"(?:扣了?|付了|支付了|扣钱了|付过|钱已经?(?:扣|付|出)了?)(?:钱|款)?{_W}{{0,12}}?"
                    rf"(?:订单|单子|页面|显示|状态){_W}{{0,6}}?(?:没|未|待|还是)(?:付|支付|成功|生成|下单|到账)",
     "扣款了订单没成功", "", _HYPOTHETICAL),
    ...
    ("seven_day", rf"(?:吊牌|标签|标牌|水洗标|商标){_C}{{0,3}}?(?:还在|没拆|没剪|没摘|没撕|没动|完好|都在|剪了"
     ...
     # 要有「退」这个**动作**（能退 / 退货 / 退吗）：「没用过退烧贴好用吗」的「退」不是（二轮复核 L2-2）
     "七天无理由", _RETURN_ACT, r"坏|质量|破|碎|裂|掉色|褪色|问题|瑕疵|发错|少发|漏发|多久|几天|到账"
     # 「能退烧 / 退热贴 / 退订」里的「退」不是退货（整合期 p14 裁定，二轮复核 L2-2 余项）
     r"|退烧|退热|退订|退出|退火|退群|退休|退税|退房|退租|退会"),
    ("return_steps", rf"(?:先|第一步|首先){_C}{{0,10}}?(?:还是|再|然后|之后)"
                     ...
                     r"|怎么搞|怎么办理|怎么操作",
     # 「退订短信 / 会员退订 / 退出」的「退」不是退货（复核 L3-1）
     # 要有退货的名词或动作（退货 / 退东西 / 寄回 / 售后）：「退热贴 / 退火炉怎么弄」不是（二轮复核 L3-1）
     "退货流程", r"退货|退款|退换|退东西|退回|退掉|寄回|售后|" + _RETURN_ACT,
     r"坏|碎|破|质量|运费|邮费|快递费|多久|几天|到账|退订|退出|退学|退休|退烧|退热|退火|退群|退役|退房|退租|退税|退会"),
    ...
    ("quality_exchange", r"掉色|褪色|染色|串色|起球|开线|脱线|变形|缩水|开胶|脱胶|起皱|发黄|生锈|漏水|漏电|坏了"
     ...
     # 「坏了想换个新的 / 推荐一台」是要买新的，不是质量换货（复核 L3-1）
     # 「换」要换的是商品本身：换货 / 换同款 / 分句末的「换 / 换一件 / 换个」（「能换吗」「可以换一件吗」）。
     # 「换电池 / 换灯泡 / 换根线 / 换系统 / 换个模式 / 换季 / 换个问题」换的是别的（二轮复核 L2-1 / L3-1）。
     "质量问题换货",
     r"换(?:货|同款)|换(?:一|个|一个)?(?:件|个|双|条|台|只|套|副|部|下)?(?=$|[吗么嘛不呢吧啊呀,，。.!！?？;；])",
     r"地址|进度|到哪|审核|多久|几天|快递|物流|推荐|换个新|换一个新|换台新|换一台新|换新|新的|买个|买一|旧"),
    # 配送范围：要有「送 / 发 / 寄」这类配送动作**和**地点 / 可达的线索（复核 L2-2 / L3-1：「送去的是正品吗」
    # 「香港能买到这款吗」「国外可以用这个充电器吗」都不是在问送不送得到）。
    # 目的地与句末问词之间不许有「的」（「寄到国外的明信片好看吗」问的是明信片，二轮复核自查）
    ("delivery_scope", r"(?:发|送|寄|配送|派送|发货)(?:得|的)?(?:到|去|往)(?![哪了货付])"
                       r"[^,，。.!！?？;；的]{1,8}?(?:吗|么|嘛|不|没)(?=$|[,，。!！?？呀啊呢])"
                       # 地名一侧只认**目的地**问法：地名后跟「能 / 可以 / 也 / 都 … 发 / 送」且动词在分句末，
                       # 或地名紧跟「发 / 送 … 吗」，或「能到 / 送得到」。「西藏发货的牦牛肉干正宗吗」「新疆寄来的
                       # 红枣好吃吗」是产地，不是送达地（二轮复核 L3-1）。
                       rf"|{_REGION}{_C}{{0,4}}?(?:能|可以|能不能|可不可以|支持|也|都)"
                       r"(?:发货|发|送|配送|派送|寄|快递)(?:货|到|得到|过去|得了)?"
                       r"(?=$|[吗么嘛不呢吧啊呀,，。.!！?？;；])"
                       rf"|{_REGION}(?:发货|发|送|配送|派送|寄|快递)(?:到|得到|得了)?(?:吗|么|嘛|不)"
                       rf"|{_REGION}{_C}{{0,6}}?(?:能到|到得了|送得到|发得到|寄得到)",
     "能不能送到",
     r"偏远|乡|农村|镇|村|山区|国外|海外|境外|港|澳|台湾|西藏|新疆|内蒙|青海|宁夏|甘肃|海南|岛|省|市|县|地区"
     r"|地方|全国|外地|哪些|这边|那边|我们这|我这|学校|单位|部队",
     r"了吗|了没|到哪|什么时候|多久|几天|啥时候|几号|哪天|明天|今天|后天|地址|包邮|运费|邮费"
     r"|正品|保修|发票|贺卡|质量|礼物|能用|可以用|买到|推荐|电压|能喝|能吃"),
    # 「用京东支付 / 京东白条 / 邮政储蓄卡 / 走京东买」是付款与购买渠道，不是选快递（复核 L3-1）
    ("courier", r"(?:什么|哪家|哪个|啥|哪种|哪一家|哪些)(?:快递|物流|快运|配送|货运)"
                ...
                r"(?!支付|付|白条|储蓄|银行|钱包|金融|商城|自营|app|买|会员|plus)"
                rf"|大件(?:商品|家具|家电|物品|东西)?{_C}{{0,4}}?(?:走|用|发|送)",
     "用哪家快递", "",
     r"到哪|了吗|了没|单号|查|进度|多久|几天|什么时候|运费|邮费|包邮|支付|付款|白条|储蓄卡|便宜|价格|分期"),
    # 「发」后面是工资 / 红包 / 消息 / 发布 / 发现一类的不是发货（复核 L2-2）
    ("ship_time", rf"(?:最快|最早|最迟|最晚|一般|大概|大约)(?:哪天|哪一天|几号|什么时候|啥时候|多久|几天|何时){_C}{{0,4}}?"
                  rf"(?:{_SHIP_VERB})|(?:今天|今晚|明天|当天)(?:下单|拍|买|付款)?{_C}{{0,6}}?(?:能|可以|会)?"
                  rf"(?:{_SHIP_VERB})(?:货|出|走)?(?:吗|么|嘛|不)",
     "什么时候发货", "", r"我的|我那|我这|那单|这单|订单|单号|了吗|了没|还没|怎么还"),
    # 主语要是退款（「退出后怎么回到原来的界面」不是，二轮复核 L2-5）；句里有「没 / 未」的是钱没到，
    # 不是问退到哪（那是退款没到账，要看这一单）
    ("refund_route", rf"(?:退款|退的钱|退回的钱|退回来的钱|退钱){_C}{{0,6}}?(?:退到|退回|回到|返回|原路|到)"
                     r"(?:哪|哪里|哪儿|什么地方|原来|原|余额|零钱|银行卡|卡上|卡里|花呗|信用卡|钱包|账户|支付宝|微信)",
     "退款退到哪", "", r"了吗|了没|到账了|没|未|怎么还|退出"),
    ("wallet_duration", r"(?:钱包|零钱|余额|花呗|信用卡|银行卡|储蓄卡|微信|支付宝|云闪付|白条)(?:支付|付款|付的|付)?"
                        rf"{_C}{{0,8}}?(?:多久|几天|多长时间|什么时候|啥时候|几个工作日|多少天)",
     # 要说的是退款（「退订花呗后多久生效」不是，二轮复核 L3-1）
     "退款多久到", r"退款|退钱|退的|退回|退到", r"了吗|了没|还没|怎么还|一直没|退订"),
)
