...
| 发现日期 | Phase | 问题 | 影响 | 建议处理时机 |
|---|---|---|---|---|
| 2026-08-30 | P7 | 🔴 **该实例不支持 SSL，公网链路是明文**。三处互相印证：客户端侧 `sslmode=require` 直接被拒（`server does not support SSL, but SSL was required`）、`sslmode=prefer` 连上但 libpq 报 SSL 未生效、服务端 `SHOW ssl` = `off`。而 `sslmode=prefer` 是 libpq 的**缺省值**，语义是「能加密就加密，不能就明文连」—— DSN 里不写 `sslmode` 时**不会有任何提示** | 查询语句与查询结果全程明文经公网传输，中间人可读可改。口令本身不是明文过线（`password_encryption` = `md5`，走挑战应答），但 md5 早已不足以对抗离线爆破。**开启 SSL 之前用过的口令都应视为已在公网暴露** | 控制台开启 SSL 加密 → DSN 改 `?sslmode=require` → 轮换口令。本轮不做：这是控制台侧动作，且开启会有一次连接闪断。口径已写进 `deploy/polardb-live.md` §3.5 |
...
| 发现日期 | Phase | 问题 | 影响 | 建议处理时机 |
|---|---|---|---|---|
| 2026-08-30 | P5 | 🔴 **冻结契约 B 写死的「有库 932」已经不成立，实测 944**。契约 B 同时要求两件互相冲突的事：T26 要新增 `test_refund_migration*.py`（派单 §5.2），而任何新增测试都会把有库全量从 932 抬高。本轮新增 12 条，于是 903+12=915（无库）、932+12=944（有库） | `scripts/demo_preflight.sh` 的 `EXPECT_TESTS` 有库分支若按契约 B 写死 932，会当场对不上并被当成回归报假警 —— 而契约 B 明文要求 T23「不要因为自己跑出别的数就去改这个契约，对不上就停手报告」。**T23 那侧照做就一定卡住**，这是契约本身的缺陷不是任一轨的执行问题 | 由人类裁决改哪一侧。建议把契约 B 的判据从写死的数字改成对新增测试免疫的**不变量**：`有库 passed == 无库 passed + 29 且 有库 skipped == 0`。这条表述钉的正是契约 B 真正要买的东西（22 条 live + 7 条 parity 一条都不许被饿死），而且以后谁加测试都不用回来改契约。本轮实测：915 + 29 = 944 ✅ |
