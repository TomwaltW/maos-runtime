...
**同一套前缀口径也认真房间里拖进来的附件** —— 判单四级里的第 ① 级就是文件名前缀
（`docs/matrix-room-runbook.md` §10.6）。所以把 `ORD-2026-0004-rust.png` 拖进
Element，它会挂到那一单上并触发一轮**只读复检**：五岗重说一遍、收口卡重算一次，
钱一分没动，放行仍要 `/approve`。

冒烟与真房间走的是同一段代码（`AttachmentStore.put()` 内容寻址落盘），
所以白名单、digest、同图去重两边逐字一致，不会出现「冒烟里配得上、房间里配不上」。

想看「补了证据结论会不会变」而又不想起 Synapse：

```bash
python3 scripts/room_team_smoke.py --evidence scenarios/custom/evidence --recheck
```

配得上证据的那几单先无证据过一轮、再带证据过第二轮，末尾并排给出两轮结论
（`第 1 轮 need_more → 第 2 轮 approve`）。不带 `--recheck` 时演的是另一件事：
**一开始就带着证据**，只有一轮。
