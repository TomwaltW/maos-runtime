...
python3 scripts/make_evidence.py         # 落 evidence/scenario-1..7，实测 4.6s
python3 -m maos.kb.experiment            # 落 evidence/scenario-R5，实测 0.7s
python3 scripts/verify.py                # 应 7/7 PASS，exit=0
```

🔴 **跑完前置，工作区一定是脏的。** `make_evidence.py` 会重写
`evidence/scenario-1..7`，`kb.experiment` 会重写 `evidence/scenario-R5` 的 7 个文件
...
🔴 **屏幕上不止七行。** 实测这七行之间夹着 **17 条 `· warn:` 缩进行**
（`trace-tree` 下 13 条、`business-outcome` 下 4 条），刷得比较长。
...
Y 轮四轨合并后，本文件要按下表逐条回填。**每一条都要重新实跑确认，不要照着「应改成」直接抄。**

| # | 镜号 | 现在写的是什么 | Y 合并后应改成什么 | 依据哪一轨 |
| :-- | :-- | :-- | :-- | :-- |
| 1 | 录制前置 | 6 条命令，`make_evidence.py` 与 `kb.experiment` 分两条敲 | 合成一条复现全量证据的命令，前置从 6 条变 5 条；耗时表下方「参考」两行的实测值跟着重测 | Y-3 |
| 2 | 00:52 | R5 对照实验：命中 4 条（首条 0.719031）、漏斗 17→9→5 | R5 数字若随语料变动照实重贴整段 `run.log`；若决定加演场景 6 的 RAG（当前 `candidate_count=0`），本镜要重新掐表并重排时间轴 | Y-2 |
| 3 | 02:06 | **A 版**：问 3 次 → 直接转人工；第二段念词只口头带过「机制在、19 条测试守着」 | 切 **B 版**：删掉 A 版第二段念词、换成 B 版增量那段；屏幕上演系统自己换渠道重试、达上限才转人工。该镜 40s → 62s，其后镜 6/7/8 顺延 22s，总长 4:25 → 4:47。**B 版屏幕输出当前无实测值，必须现跑后照实补贴** | Y-4 |
| 4 | 02:06 | 「三件事」表第 2 行结论：机制在、屏幕上演不出来、用 A 版 | 改成「已能演」，并把该镜「A 版严禁做的事」那条 🔴 整块删掉 | Y-4 |
| 5 | 03:24 | 「屏幕上不止七行，夹着 17 条 warn」+ 滚到 `RESULT` 再停 | `business-outcome` 下 4 条 warn 消失后重数 warn 条数；若归零则整段「不止七行」提醒删掉，镜头可直接给七行 | Y-1 |
| 6 | 03:24 | 七行 PASS 的实测值（`hash-integrity 74/74`、`business-ref 33/33`、`kb-hit 4/4` 等） | 证据束下一轮重跑后数字会变，照实重贴七行 + `RESULT` 行 + 「证据来源」行 | 整合轮 5 证据束重跑 |
| 7 | 03:57 | 区间 `90251b3..4a70cb0`，`contracts/` 与 `core/` 双空 | **端点不要动**（与 Z-3 的 `docs/domain-portability.md` §2 共用同一对）；只在两边措辞上对齐一次 | Z-3 对齐 |
| 8 | 耗时表 | A 版 265s / B 版 287s，按 4 字/秒折算 | 上述任何一镜念词改动后，重数字数、重算「念完约需」与「富余」，确认总长仍在 3–5 分钟 | 全部 |
