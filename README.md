...
> **第一次拿到这个包、想先看效果？** 双击 `START-HERE.html`，或读
> [`README-CLIENT.md`](README-CLIENT.md) —— 那是给业务读者的版本，不用敲任何命令。
> 本文是另一份：给要读代码、核对证据的技术读者。
...
> 🔴 **克隆时别加 `--depth 1`。** 第 9 项要回答「这批证据是不是当前这份代码跑的」，而证据
> 入库这个动作本身会让 HEAD 前进一格（证据恒自称上一个 sha）。判据因此不是「sha == HEAD」，
> 是「那一格之后没动过 `evidence/` 以外的东西」—— 这一问要读**祖先 commit 的 tree**，
> 浅克隆压根没带那段历史。这时核验器**既不判负也不放行**，按 `[SKIP]` 计并说明怎么补
> （`git fetch --unshallow`）：判不了就说判不了，不拿「我查不了」冒充「我查过了」。
> [§4](#4-5-分钟快速开始) 那条 `git clone` 不带 `--depth`，照写就对；提交物压缩包由
> `scripts/make_release.sh` 打，克隆深度已经够第 9 项读到祖先。
...
| AgentTeams 事件链 | 圆桌五岗 + DAG 五岗六任务落在**同一条** `event_log` 上 | `evidence/case-real-01/happy/event-chain.json`（`events[]` 按 `seq` 升序，条数与分类计数都在同文件的 `count` / `by_type` 字段里，不在这里写死）＋ 零模型回放器 `scripts/replay_roundtable.py`。DAG 那一侧数 `evidence/case-real-01/happy/result.json` 的 `plans[0].tasks`：**6 个任务、5 个角色**（受理岗担两个任务 —— 受理与通知；政策、渠道、财务、付款各一） |
