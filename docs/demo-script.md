...
第 1 条已在 T53 轮按 `36bd036` 重核）：Element 房间的状态（**真房间已接通** ——
房间实拍是默认路径，纯终端降级为应急回退）、审批由场景脚本自动驱动。
...
**第 2／3 行按基线 `27c9e18` 逐条实跑 / Read 代码复核（2026-08-29 · T 轮），不是照抄旧结论。
第 1 行已在 T53 轮（2026-09-01）按基线 `36bd036` 重核并整格改写** —— 旧结论「证据未到位」
已被本仓自己的证据推翻，照旧稿讲会当众自曝一个错误结论。

| # | 事项 | 复核结论（`27c9e18`） | 影响哪一镜 | 处置 |
| :-- | :-- | :-- | :-- | :-- |
| 1 | **Element / Matrix 房间** | **又变了：基建到位，证据也到位。** T53 轮在 `36bd036` 上实测 `evidence/room/`：5 张 Element 截图（`01`–`05`）+ `README.md` + **861 行**逐字副本 `transcript.md`（出处头 `27c9e18`，从 Matrix client-server API 的房间消息历史整份拉下来，**不是 stdout**）。三条路径各在真房间实测跑过一次：`/approve`→`BLOCKED → DONE`、`/reject`→`BLOCKED → FAILED`、名单外 `@intern` 两次被拒且闲聊零回复。⚠️ **前置一条都没松**：本轮实跑系统 `python3` 的 `import nio` **仍是 `ModuleNotFoundError`**，`_NioChannel`（`hiclaw/matrix_bus.py::_NioChannel`）拿它跑恒走 `ImportError` → 降级 `log_only`；活路径**只在** `~/.maos-matrix/venv/bin/python`（matrix-nio 0.26.0 装在那儿）。`~/.maos-matrix/STATUS` 本轮实测 `READY` | 手册原分镜的 00:00「房间里看拆解」与 02:00「审批卡 /approve」；本分镜的 **AgentTeams 状态展示**这一必含项 | **A/B 双分支已塌缩，见下一节。** 房间实拍**转正为默认路径**；原 A 分支（纯终端）降级成**应急回退**，只在录制当天房间起不来时才走 |
...
## 第 1 件事：AgentTeams 状态展示怎么录（房间实拍为默认，纯终端为应急回退）

> 🔴 **本节 T53 轮（2026-09-01）整节改写。** 上一版写的是「**A 保底，今天就能录；
> B 更强，等 T4**」，并挂着一条红线「B 分支一个字都不许提前写进念词」。
> **那条红线的触发条件已经不成立** —— T4 轨早就交了：`evidence/room/` 下 5 张 Element
> 截图 + 861 行逐字副本，全部在 `36bd036` 上，不在别的分支等着。
>
> 改法不是把红线删掉了事。旧的 A 分支**不是一个中性备选** —— 它是在更强证据已经到位
> 之后仍然存在、且被门禁**默认选中**的那条路；留着它不动，录制当天大概率还是走它。
> 所以：**房间实拍转正为默认路径，原 A 降级成应急回退，并写死触发条件。**
> 没有直接把终端那条删掉，是因为它是真实存在的降级能力 —— 删了就演不出
> 「房间挂了流水线照跑」这条主张。已记 `docs/DECISIONS.md` 的 `## task-T53`。

### 默认路径 · 房间实拍（原 B 分支，现已转正）

**开录前置**（三条全绿才开录；任一条不绿走下面的应急回退）：

1. 🔴 **解释器必须是 `~/.maos-matrix/venv/bin/python`。** 系统 `python3` 实测
   `import nio` → `ModuleNotFoundError`（T53 轮重跑仍如此），拿它起房间会
   **静默降级 log-only** —— 终端照常刷「房间消息」而房间里一条都没有。
   **这是本节最容易翻车、也最容易被误判成「录成功了」的一步。**
2. `cat ~/.maos-matrix/STATUS` → `READY`（T53 实测 `READY 2026-08-29T05:52:11Z`）；
   `docker ps` 里 `maos-synapse` / `maos-element` 均 healthy。
3. Element（`http://localhost:8080`）以 `@boss` 登入，房间「MAOS 审批」可见。
   🔴 **只截浏览器视口**，不截整屏 —— 地址栏 / 标签页 / 书签栏 / 桌面一律不入画
   （access token 在 Element 设置页里，录制中一次都不要打开它）。

**镜头怎么改**（左列是回退时的旧做法，右列是现在的默认做法）：

| | 应急回退（纯终端） | **默认 · 房间实拍** |
| :-- | :-- | :-- |
| 镜 1 | `python3 run.py --scenario 7`，看终端 Plan 表 | `~/.maos-matrix/venv/bin/python run.py --scenario 7 --matrix`，**画面切 Element 房间**，看事件逐条刷进来 |
| 镜 4 的审批停点 | 终端轨迹里的 `gate_needs_human` → `human_approve` | 房间里**现场手打** `/approve <task_id>`，镜头给回执 `已批准 …（操作人 @boss:…）` |
| 镜 6 的驳回 | 终端轨迹里的 `human_reject` | 房间里现场 `/reject <task_id> 回执非终态`，镜头给 `BLOCKED → FAILED` 与 Plan `RUNNING → FAILED` |

🔴 **镜 6 的念词不许说「镜头给补偿执行」**（旧稿的 B 分支这么写过，是错的）。
`CompensationExecuted` 走 `store.append_event_log()`、**从不 publish**，永远不进房间；
成功路径上终端也一行日志都不打。`04-reject-compensation.png` 的**文件名比它能证明的
东西大** —— 它证明的是「驳回生效 + Plan 落 FAILED」，不是补偿。补偿的证据在
`evidence/scenario-7/`（机器侧、有库、进 `scripts/verify.py` 核验）。
依据是 `evidence/room/README.md` 里「🔴 `04` 的文件名比它能证明的东西大」那一节，
**本节口径不许比那一节更满**。

**念词怎么改**：把回退版那句「今天演的是权威那一路」换成
...
🔴 **别念「房间镜像稳定可靠」。** Synapse 默认限流实测打穿过：一轮 approve 出过 4 条 429，
`_NioChannel` 的 10s 超时会因此误报「房间回话失败（）」而消息其实已送达。
台上真撞见了就照这句解释，别慌着重发。

### 应急回退 · 纯终端（原 A 分支）

🔴 **只有一个触发条件：录制当天房间起不来。** 具体指 `~/.maos-matrix/venv` 不可用、
`maos-synapse` 容器起不来、Element 连不上 —— 三者任一。
**不许因为「终端更稳」「省事」走这条。** 更强的证据已经在库里，主动走回退是自弃一分。
...
🔴 **走回退时，念词必须补这一句**（不占额外时长，塞进镜 4 的富余里）：

> 「今天屏幕上走的是**降级路径** —— 房间是旁路，权威记录在 `event_log` 里，
> 降级模式与房间模式的 publish 序列有测试断言等价。**真房间的证据在 `evidence/room/`**：
> 5 张 Element 截图和一份逐字副本，`/approve` 生效、`/reject` 生效、名单外越权被拒，
> 三条都在真房间里实测跑过。」

**这句是回退路径的硬要求，不是加分项。** 不说就等于让评委以为我们只有终端；
而 `evidence/room/` 就摆在仓库里，评委 `ls` 一下就看得见，届时反而更难解释。

🔴 **不要摆拍一个假房间**，回退时也不要加 `--matrix` 演 —— 拿系统 `python3` 跑它只会
走降级 `log_only`，屏幕上没有房间，而那份终端输出与真房间证据**形态一模一样、无法分辨**。
这条红线原样成立，且正是 `evidence/room/README.md` 那节「为什么这份证据可信：
它不是从终端抄的」要防的事。

> **本轨（T53）不改 `evidence/room/**`、`docs/matrix-room-runbook.md`、
> `docs/agentteams-mapping.md`** —— 上面对房间现状的判断全部来自对 `evidence/room/`
> 的**只读核对**，以及本轮在 `36bd036` 上对解释器与 `STATUS` 的实跑。

## 与手册原分镜的对照

手册（`docs/EXECUTION.md:744` 起 —— 上一版写的 `:724` 已过期）给的是 8 个镜头，
本分镜是 **9 镜**。差异三处，都有依据，**没有一处是删戏**；
其中第 3 处**已在 T53 轮消解** —— 房间画面回到了手册原来的做法，剩下两处仍是差异：
...
| 3 | ~~手册的 Element 画面全部改走终端~~ **已回到手册：Element 画面就是默认路径** | 见「三件事」第 1 条。截图已到位（`evidence/room/` 5 张 + 861 行逐字副本），T53 轮已把房间实拍转正；纯终端只剩**应急回退**一条用途，触发条件写死在那一节 |
...
| 10 | 三件事 · 第 1 条 | 一段话说房间还没接上 | **A/B 双分支**，各自掐表；A 当时可录，B 须等房间截图到位 | 当时实测：`hiclaw/room_demo.py` 已存在，但系统 `python3` 的 `import nio` 报错、`evidence/room/` 还是空的。⚠️ **本行是 2026-08-29 的历史记录，其结论已于 T53 轮（2026-09-01）被推翻** —— T4 轨已交付 5 张 Element 截图 + 861 行逐字副本，双分支随之塌缩成「房间实拍默认 / 纯终端应急回退」，见「三件事」第 1 条与紧随其后的那一节 |
...
| ~~3~~ | ~~**AgentTeams 的 B 分支**~~ | ✅ **已结（T53 · 2026-09-01）。** 真房间证据已交付（`evidence/room/` 下 5 张 Element 截图 + 861 行逐字副本），双分支已塌缩：房间实拍转正为默认路径，纯终端降为应急回退。**剩下的不是等，是掐表** —— 切房间后总长约 5:38~5:44，须从镜 3、镜 5 的富余里借 6s 压回 5:30 附近，录制当天实测一次 |
