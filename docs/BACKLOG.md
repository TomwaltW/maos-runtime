...
| 日期 | Phase | 现象 | 影响 | 建议处理时机 |
|---|---|---|---|---|
| 2026-09-14 | p10 | **交付包里 pytest 恒红 3 条**：`test_docs_guard.py::test_docs_structure_and_references_are_sound`、`test_rtv_sop_doc.py::test_two_docs_pass_check_docs`、`test_rtv_sop_doc.py::test_check_docs_cli_still_exits_zero`。根因是 `docs/` 多处引用 `review/` 下的文件（`review/rtv-contracts.md`、`review/refund-skill-contracts.md` 等），而 `review/` 被 `.git/info/exclude` 排除、**不入版本库** —— clone 出来那些路径不存在，文档守卫报 **54 条 `E-missing`**。worktree 里全绿，只在 clone / 解压包里红 | `scripts/make_release.sh` 第 2 条口径（解压后 pytest 全绿）过不了，**非 0 退出、产不出可提交的包**。T153 把 provenance 修好之后，这条是打包路上仅剩的两块拦路石之一 | 复赛前必须处理，不属 T153 白名单。两条路各有代价：把被引用的 `review/*.md` 收进版本库（要动仓库边界与 `.git/info/exclude`），或让文档守卫对「已知不入库目录」的引用降级成非阻断。**别直接删文档里的引用** —— 那几处指向的是真的内部契约文档，删掉等于丢线索 |
...
| 2026-09-14 | p10 | `scripts/make_release.sh` 第 2 条口径里那句 `[FAIL] 解压后生成的证据带 -dirty`（实测恒 3 个）**名不副实**：它数的是 `grep -rl '^# generated at.*-dirty' evidence/`，即包内**所有**带 `-dirty` 首行的文件，而那 3 个都是入库时就带着的历史产物、不是这次解压后生成的 —— `evidence/capability-matrix.json`（`e0541e9-dirty`，p9 独立维护）、`evidence/room/roundtable-four-outcomes.json` 与 `.txt`（`f4ffa76-dirty`，人工采集无生成器） | 与 `verify.py` 口径打架：第 9 项对这两类是**认下的**（room 在 `_MANUAL_BUNDLES` 里 warn 豁免，`capability-matrix.json` 压根不进 provenance 分母），打包脚本却拿同一批文件判负。后果是 `make_release.sh` 非 0 退出，**且这条恒红**——fb2100b 那次跑也是同样的 3 个。三条 FAIL 里这是第三条 | 复赛前处理，不属 T153 白名单。修法：把判据收窄成「**本次 `make_evidence.py` 产的**束带 `-dirty`」（对齐 `verify.py::provenance_anchors` 的束级口径与 `_MANUAL_BUNDLES` 豁免），而不是扫整个 `evidence/`。**别直接把阈值从 0 改成 3** —— 那会让真正的「解压后工作区不干净」再也报不出来 |
