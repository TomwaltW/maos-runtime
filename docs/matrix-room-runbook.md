...
前面九节演的是**一个** bot（`maos-bot`）把事件镜像进房间。这一节演的是另一件事：
一单退款进来，**五个岗位以各自的 Matrix 账号**依次发言，每一位接着上一位的话说 ——
申请受理岗 → 规则审核岗 → 证据核验岗 → 风险反欺诈岗 → 财务执行岗。

事实全部由规则代码（skill）算出来，模型只负责把事实说成人话，五个岗共用同一个
`MAOS_LLM_*` 客户端。没配模型也**不沉默**：直接发事实卡。

### 10.1 先在本机看一眼形态（不需要 Synapse）

```bash
python3 scripts/room_team_smoke.py
```

零网络、零模型、零环境变量，也因此不会有任何口令出现在输出里。圆桌引擎还没并进来时
它打一行「圆桌引擎未装载」并以 `3` 退出 —— 这是**设计好的退化路径**，不是故障。

### 10.2 三步进真房间

```bash
bash deploy/synapse/add_agents.sh                     # 建 5 号、进房、写 ~/.maos-matrix/agents.env
set -a; . ~/.maos.env; . ~/.maos-matrix/room.env; . ~/.maos-matrix/agents.env; set +a
~/.maos-matrix/venv/bin/python -m hiclaw.room_ingress
```

🔴 **三份 env 一份都不能漏**：`~/.maos.env` 给模型，`room.env` 给房间，
`agents.env` 给五个岗位账号。`agents.env` 只在仓库外（`~/.maos-matrix/`、`chmod 600`）、
永不入库；本手册只写**键名与 mxid**，一个值都不写。

解释器同 §0：必须是那个 venv，系统 `python3` 没装 `matrix-nio`。

### 10.3 boss 在 Element 里的动作序

| 打什么 | 期望看到 |
| :-- | :-- |
| `/team` | 零模型列出五个岗位、各自的 mxid 与 skill 清单 |
| `/refund ORD-2026-0004 质量问题` | 先一张预检卡，随后五岗依次发言，末位给出 `/approve` 提示 |
| `/approve RC-ORD-2026-0004` | 财务执行岗汇报核算、付款受理与到账观察 |

`ORD-2026-0004` 是本轮新加的演示订单（底账 `scenarios/custom/ledger.json`）。
另两单 `ORD-2026-0005` / `ORD-2026-0006` 与五种剧情的对应关系写在
`scenarios/custom/README.md` 的「用法三：五岗圆桌演示」。

### 10.4 期望看到什么

- **五条发言，发言人是五个不同账号**（`@maos-intake` / `@maos-policy` /
  `@maos-evidence` / `@maos-risk` / `@maos-finance`），而不是 `maos-bot` 挂五个名牌。
- 某一岗的号没接通时，它退化成 `maos-bot` 代言、正文前挂
  `【岗位 · agent_id】` 名牌。**这是设计，不是故障** —— 缺一个号不该让整轮圆桌哑掉。
  启动时那一行 `describe()` 会写明哪几岗独立、哪几岗代言、为什么。
- 放行**前**，财务执行岗说的是**核算预演**：算给人看，没落账，放行后按同一段代码正式核算。
  放行**后**说**已受理**。钱的下落归网关：只有付款观察真问出终态，才会出现到账那句话。
  屏幕上看到「预演」不是没算，是还没落账。

### 10.5 boss 视角：一单从头到尾，屏幕上依次出现什么

前面几节是给搭台的人看的。这一节是给**在 Element 里看结果的人**看的：一单退款
从起单到拍板，屏幕上按顺序出现哪几块东西，每一块该读哪一行。

#### 起单：两条路，形态一样

| 打什么 / 做什么 | 屏幕上出现 |
| :-- | :-- |
| `/refund ORD-2026-0004 质量问题` | 一张只读**预检卡**：裁定、依据（规则号 + 政策版本）、距付款天数、申报金额、怎么放行 |
| 先拖一张照片进房间，再打上面那句 | 同上，且受理岗那条会写「随案证据：1 份」—— 照片是**先发后认领**的，`/refund` 一来就挂到这一单上 |
| 拖一张申请表 CSV 进来 | 每岗只汇总一次的整表结论（不逐行五连发，50 行 × 5 岗会把房间刷爆） |

预检卡是**回帖发完之后**五岗才开口的 —— 裁定不等五次模型调用。

#### 五岗依次发言

一岗一条，顺序固定：申请受理 → 规则审核 → 证据核验 → 风险反欺诈 → 财务执行。
每一位接着上一位的话说。没配 `MAOS_LLM_*` 时发的是**事实卡**原文（规则代码算出来的
那份），不是模型复述的人话 —— 不沉默、也不刷一句 `{}`。

要看的就三行：规则审核岗的**裁定**、证据核验岗的**结论与缺口**、风险反欺诈岗的**档位**。
财务执行岗给的是**核算预演**（未落账）。

#### 收口卡：五岗说完之后，谁对你说「批还是不批」

五条结论互相矛盾时谁说了算 —— 这就是收口卡回答的问题。它**不归五岗任何一岗**
（让财务岗来念会让人以为是财务的判断），由主通道 `maos-bot` 发一张卡片：
首行一句话结论，其后逐条依据、拦路条，末行下一步该打什么命令。

结论只有四种。下面四张卡**逐字**来自本机 2026-09-03 收口轮四轨整合之后的实跑
（`python3 scripts/room_team_smoke.py --evidence scenarios/custom/evidence`），
同一份输出落在 `evidence/room/roundtable-four-outcomes.txt`。
房间里那张卡多一圈边框（Element 里是 `<blockquote>`），内容与这里逐字相同。

**① 建议批复**（ORD-2026-0004：证据齐、风险 low、核算预演算得出）

```
建议批复 · 核准预演 9600.00 · 请 supervisor 拍板
  · 申请受理岗：订单 ORD-2026-0004，实付 9600.0，申报 9600.0，随案证据 1 份
  · 规则审核岗：裁定 approve，依据 无单条规则决定（命中 3 条，政策 v1）
  · 证据核验岗：核验结论 complete，缺口 0 项，随案材料 1 份
  · 风险反欺诈岗：风险 low（评分 10），命中 1 条信号
  · 财务执行岗：核算预演 9600.00（政策 v1），未落账
下一步：/approve RC-ORD-2026-0004
```

**② 建议升级审批**（ORD-2026-0006：证据也齐，但风险 high）

```
建议升级审批 · 风险 high · 请 finance_manager 复核
  · 申请受理岗：订单 ORD-2026-0006，实付 88000.0，申报 92000.0，随案证据 1 份，申报高于实付
  · 规则审核岗：裁定 approve，依据 无单条规则决定（命中 3 条，政策 v1）
  · 证据核验岗：核验结论 complete，缺口 0 项，随案材料 1 份
  · 风险反欺诈岗：风险 high（评分 100），命中 5 条信号
  · 财务执行岗：核算预演 88000.00（政策 v1），未落账
  ⚠ 风险：订单 ORD-2026-0006 在退款底账里已有处理中或已退成的记录，属重复退款申请
  ⚠ 风险：订单 ORD-2026-0006 已有一笔退款到账（settled），本次是对同一单的二次退款
  ⚠ 风险：同一客户近 30 天内已有 3 笔退款记录，频率偏高
下一步：/approve RC-ORD-2026-0006
```

审批人从 `supervisor` 升到了 `finance_manager` —— **风险岗只提示不裁定**，
所以它把结论推高一档、不推翻。

**③ 需补件后再议**（ORD-2026-0005：规则说能退，但一张图都没有）

```
需补件后再议 · 证据：缺少 image 类证据
  · 申请受理岗：订单 ORD-2026-0005，实付 1980.0，申报 1980.0，随案证据 0 份
  · 规则审核岗：裁定 approve，依据 无单条规则决定（命中 3 条，政策 v1）
  · 证据核验岗：核验结论 missing，缺口 2 项，随案材料 0 份
  · 风险反欺诈岗：风险 medium（评分 30），命中 2 条信号
  · 财务执行岗：核算预演 1980.00（政策 v1），未落账
  ⚠ 证据：缺少 image 类证据
  ⚠ 证据：证据份数不足：要求 1 份，实收 0 份
  ⚠ 风险：同一客户近 30 天内已有 2 笔退款记录，频率偏高
下一步：补齐材料后重发 /refund <订单号> <诉求类型>
```

🔴 **缺件是可补的，不是拒绝。** 举证不足只让免责条款不予适用，不构成拒赔 ——
把「没交图」判成「不给退」是本域最严重的做错方式。

**④ 不建议批复**（ORD-2026-0005 七天无理由：第 58 天才申请）

```
不建议批复 · AS-001@v1 窗口 30 天，第 58 天申请，58 > 30
  · 申请受理岗：订单 ORD-2026-0005，实付 1980.0，申报 1980.0，随案证据 0 份
  · 规则审核岗：裁定 reject，依据 AS-001@v1（命中 3 条，政策 v1）
  · 证据核验岗：核验结论 not_required，缺口 0 项，随案材料 0 份
  · 风险反欺诈岗：风险 medium（评分 30），命中 2 条信号
  · 财务执行岗：未做核算预演（裁定驳回或前置不成立），无金额
  ⚠ 风险：同一客户近 30 天内已有 3 笔退款记录，频率偏高
  ⚠ 风险：同一客户名下有 3 笔订单快照，存在批量退款的可能
下一步：/reject RC-ORD-2026-0005 <理由>
```

规则说不退就是不退：证据齐、风险低都不能把它翻成批准 —— 那是拿观察去改裁定。

#### 拍板：`/approve` 之后

财务执行岗再说一句：核准金额、政策版本、业务状态、**到账观察几条**。

🔴 **收口卡说的是「建议」，不是「已经」。** 房间里永远不会出现「已批准」「已退款」
「已到账」这类词，除非付款观察真的问出了终态。钱的下落归网关，MAOS 只持有观察
（铁律 8）。屏幕上看到「预演」不是没算，是还没落账。

#### 不需要 Synapse 也能把这四张卡看一遍

```bash
python3 scripts/room_team_smoke.py --evidence scenarios/custom/evidence
python3 scripts/room_team_smoke.py --evidence scenarios/custom/evidence --pace 600   # 录屏用
```

`--evidence` 把 `scenarios/custom/evidence/` 里的图按文件名前缀配给订单
（`ORD-2026-0004-*.png` 配给那一单），走的是与房间里拖附件**逐字相同**的那条路。
不给这个参数时每一单的证据都是空的，四种结局里只演得出 `need_more` 与 `reject`
—— 那正是这个参数补上的那半条剧情。`--pace` 给发言之间加停顿，缺省 0。

合议引擎（`maos.roundtable.verdict`）已在 2026-09-03 的收口轮整合中并入，
上面四张卡冒烟与真房间里都看得到。冒烟脚本若打出「合议引擎未装载」，说明这个
模块不在当前工作区 —— 那是部署问题，不是退化：五岗发言与命令面此时照常。

### 10.6 证据补齐 → 五岗复检

前面演的是「起单 → 五岗 → 拍板」一条直线。这一节演的是**中途补料**：一单已经
因为「随案零证据」被判 `need_more`，boss 把照片拖进房间，五岗**重新过一遍**。

#### boss 在 Element 里逐条照做

① `/refund ORD-2026-0005 质量问题`

五岗第 1 轮：证据核验岗报「随案零证据」、缺口 2 项，收口卡 `need_more`，
末行写的是「补齐材料后重发」而不是任何一条放行命令。

② 把一张图拖进房间（文件名随便，或者叫 `ORD-2026-0005-rust.png`）

房间先回一句预告：「收到新证据，五岗正在复检这一单（第 2 轮）……」——
它与首检那句**不是同一句话**，看得出这是第二遍。随后五岗第 2 轮：
证据核验岗报「本轮新增 1 份」，收口卡**可能**翻成 `approve`。

③ `/approve RC-ORD-2026-0005`

🔴 **拖图触发的是只读复检，不是放行。** 第 2 轮只是把结论按新事实重算一遍，
钱一分没动、状态一格没走。真要放行仍然要 boss 亲手打 ③ 那条命令（铁律 8）。

🔴 **是「可能」翻成 approve，不是一定。** 结论由合议真值表按五岗当轮的事实定：
补证据只解掉「证据」那条拦路条。规则驳回（`AS-001` 超出 30 天窗口）与风险 high
都不会因为多一张图改口 —— 那是拿观察去改裁定。

#### 这张图是哪一单的：判单四级

拖进来的附件挂到哪一单上，按这个顺序认，**命中即止**：

| 级别 | 依据 | 例子 |
| :-- | :-- | :-- |
| ① | 附件**文件名前缀** | 文件名 `ORD-2026-0005-rust.png` → 挂给 `ORD-2026-0005` |
| ② | 同一条消息的**正文** | 拖图时正文写「ORD-2026-0005 的锈蚀照」→ 挂给 `ORD-2026-0005` |
| ③ | 本会话**最近一条未过期待办** | 刚打完 `/refund ORD-2026-0005 质量问题`，再拖一张 `IMG_8821.jpg` → 挂给 `ORD-2026-0005` |
| ④ | 三级都不命中 | **不触发五岗**：图先存着，等下一句 `/refund <订单号> <诉求类型>` 来认领 |

第 ④ 级不是故障。房间里会回一句「收下当证据，等一句 `/refund` 认领」，图没丢，
也没有挂到别人头上 —— **挂错单比不挂更坏**：那会让另一单的证据核验岗
凭空多出一份材料，而它算出来的东西照样自洽、不报错。

一条消息带三张图 = **一次**复检、五岗各说一遍，房间里也只有**一句**预告。
看见五岗说了三遍，那是 bug，停下来报告。

#### 不起 Synapse 也能把这两轮看一遍

```bash
python3 scripts/room_team_smoke.py --evidence scenarios/custom/evidence --recheck
```

配得上证据的那几单（`ORD-2026-0004` / `ORD-2026-0006`）先无证据跑一轮、再带证据
跑第二轮，末尾并排给出两轮的收口卡结论（下面这两行来自 2026-09-05 的实跑）：

```text
ORD-2026-0004：第 1 轮 need_more → 第 2 轮 approve
ORD-2026-0006：第 1 轮 need_more → 第 2 轮 escalate
```

0006 那一行正是上面「不是一定」的例子：证据补齐了，风险仍然 high，
所以它翻到的是**升级审批**、不是批复。

`ORD-2026-0005` 在演示语料里故意一张图都不配（理由写在
`scenarios/custom/evidence/README.md`），所以真房间里从头演一遍「② 拖图 → 复检」
就该拿它开刀 —— 别的单一开始就带着证据，第 1 轮没有「缺」可补。

### 10.7 排障三条

**① 房间里五岗全是 `maos-bot` 代言**

- 判据：看启动那一行 `describe()`，它点名哪几岗没接通。
- 怎么办：多半是 `agents.env` 没 source（`set -a` 那行漏了），或那个号还没进房 ——
  没 join 的账号在通道构造期就会失败。重跑 §10.2 第一步的建号脚本，再确认三份 env 都在环境里。

**② 岗位发言之后 bot 又接一句闲聊，越接越多**

- 判据：房间里出现「岗位发言 → `maos-bot` 闲聊 → 岗位再发言」的接龙。
- 怎么办：`MAOS_ROOM_BOTS` 没生效。它是逗号分隔的 mxid 名单，监听方对名单里的 sender
  一律不喂给消息回调。确认这个键在 `agents.env` 里，且**在启动进程之前**就已进环境
  （进程起来之后再 export 不算）。

**③ 发言里出现事实卡里没有的数字**

- 判据：把那条发言里的订单号 / 金额 / 规则号，逐个到同一岗的事实卡里找。找不到就是它。
- 怎么办：**停下来报告 —— 这是 bug，不是文案问题**。事实只许来自规则代码；
  靠改提示词把它遮过去，等于把一个会算错钱的缺陷改成一个不显眼的缺陷。

守着这一轮演示数据的测试：`maos/tests/test_room_team_fixture.py`；
守着 §10.6 那两轮（预告措辞、两轮结论、退化路径）的是
