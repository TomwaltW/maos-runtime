...
* **确定性优先**：语种、槽位、意图都先按规则判（触发词 → 意图示例 → 诉求 / 关键词词表）；
* 只在规则判不出意图、且 ``ctx.extras["model"]`` 是**真模型**（非 None、非 ScriptedModelClient）
  ...
  一行，``trace_id=""``、``task_id=None``、``plan_id=extras["plan_id"]``（缺了按会话现算
  ``cs:csc-…``）。记账发生在 :func:`maos.domain.cs.understand.understand` 里（契约把 model /
  store / plan_id 给了它），call_site 是那个模块的 ``CALL_SITE``；本 skill 只是把 extras 接过去。
...
class CsUnderstandSkill(Skill):
    contract = SkillContract(
        ...
        purpose="客服前台一轮的理解：判语种、抽槽位（订单号 / 商品 / 问题 / 诉求 / 情绪，跨轮合并）、"
                "判意图（触发词 → 意图示例 → 诉求与关键词词表；判不出且注入真模型才问模型）",
        input_schema={
        ...
        security_boundary="只读、不调任何工具、不写任何业务表、不落事件：槽位只从词表与正则来"
                          "（不自由抽取，客户的住址、手机号进不了槽位），11 位手机号形态不当单号；"
