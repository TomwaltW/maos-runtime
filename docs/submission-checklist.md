...
```bash
# ①
python3 -m pytest maos/tests -q          # □ 571 passed，个位数秒
# ②
...
# ⑥
python3 scripts/verify.py                # □ 7/7 PASS，exit=0，另有 10 行 warn（2 类，见 A-2）
# ⑦
...
#### verify.py 的 10 行 warn 是**已知缺口，不是回归**

`verify.py` 报 7/7 PASS 的同时会打印 10 行 `· warn:`，分 2 类（整合轮 5 实测）。
...
**已在整合轮 5 归零** —— 分别由 Y-1（执行路径进证据）与 Y-2（规划期调用归树）补掉。
17 行 / 4 类是整合轮 5 之前的读数，别照抄旧材料。

- [ ] 跑出来的 warn **就是这 10 行 / 2 类**，B 类与 C 类**不该再出现**（出现了就是回归）。
...
- [ ] PPT 与视频里出现的**每一个数字**（`571 passed` / `7/7 PASS` / `33/33` / `19 条测试` …），
...
**Y-4（场景 7 演换渠道重试）尚未合并**，下列各处等它：

| # | 位置 | 现在写的是什么 | Y-4 合并后应改成什么 |
| :-- | :-- | :-- | :-- |
| 1 | A-4 replan 行 + 其后的 ⚠️ | 「机制已落地…但演示里没有场景走这条路」 | 改成「场景 7 演换渠道重试到上限」；右列「不许这么说」那句**转正**；⚠️ 删掉 |
| 2 | §B 第 10 条 | 证据列的是闸 + 政策锁 + `MAX_REPLAN` | 补上场景 7 的实跑换渠道证据 |
| 3 | §C 的 ⚠️（Y-4 合并前不要开录） | 02:30 那一镜与「录制前三件事」第 2 件会变 | 按 `demo-script.md` 的 B 版重写该镜，⚠️ 删掉 |

**等人类补的**（与轮次无关）：

| # | 位置 | 等什么 |
| :-- | :-- | :-- |
| 4 | 全文 `OQ-1` / `OQ-2` 指针 | 官方口径到手后，按 [`docs/open-questions.md`](open-questions.md) 的回填点表逐处补，并把该文件对应条目标为「已答」 |
