...
ADDED_PATTERNS: dict[str, tuple[str, ...]] = {
    HANDOFF_ANGER: (
        ...
        # （「破东西 / 破包装」多半在说商品本身，不收）
        # 「遛狗服务 / 养狗服务」是在问商品（复核 L3-4）：「狗」前挨着遛 / 养 / 喂 / 宠 / 牧的不算
        r"(?:破|烂|(?<![遛养喂宠牧])狗|鬼|渣)(?:客服|机器人|系统|平台|服务|店铺|公司|态度|app|软件|回复)",
        ...
        "白痴", "弱智", r"蠢(?:货|猪|死|蛋)", r"废物(?!利用|回收|处理|再)", "人渣", "屁用", "狗屁",
        # 「有病 / 摆设 / 死人」只认分句末（后面紧跟语气词、标点或句末）：「你们有病号服吗」「你有病历本
        # 卖吗」「你们摆设的那个花瓶」「死人头牌」是在问商品（复核 L3-4）
        "神经病", r"(?:你们?|客服)(?:是不是|是)?有病" + _CLAUSE_END,
    ...
    # 条件必须是**明确的条件或拒绝结构**（复核 L2-1 / L3-3）：「要是 / 如果 / 若 / 假如 / 万一 … 就」，
    # 或「不 / 没 / 再不 / 还不」**紧跟处理类动词**（退 / 处理 / 解决 / 发 / 回复 / 赔 / 换 / 补 / 给 …）
    # 再「… 就」，或「否则 / 不然」。「不好意思 / 不错 / 不到一周 / 没想到」不是条件。
    # 后果一侧：「就」与动作之间不许有否定或「可以」（「就不会给差评」「就不用找平台了」「就可以给差评吗」
    # 是表态或问规则）；动作之后同一分句里有「夸 / 好评 / 吗 / 么 / 嘛 / 能删」的不算（夸奖、问句）。
    # 「我不会给差评的」「不退款我就自己留着」「不退的话我会很失望」都不算（纯发泄、拿主意、表态）。
    HANDOFF_COMPLAINT: (
        r"(?:(?:要是|如果|若是?|假如|万一)[^。.!?！？]{0,16}?就"
