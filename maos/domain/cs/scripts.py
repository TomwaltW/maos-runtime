...
#: 发货的动词（「发工资 / 发红包 / 发消息 / 发布 / 发现」里的「发」不算，复核 L2-2）。
_SHIP_VERB = (r"发(?!工资|奖金|红包|薪|邮件|短信|消息|信息|朋友圈|微博|视频|帖|票|布|现|生|表|烧|芽|言|放|证)"
              r"|寄|出库|出货")
...
SYNONYM_RULES: tuple[tuple[str, str, str, str, str], ...] = (
    ...
    ("damaged", rf"(?:箱子|盒子|纸箱|外箱|外包装|包装盒?|快递盒|快递袋|袋子|包裹){_C}{{0,4}}?"
     ...
     "包裹破损", "", r"换|质量|用了|用就|一用|用过|穿|那种|设计|款式|有没有|有吗|是不是"),
    ...
    ("delivery_scope", r"(?:发|送|寄|配送|派送|发货)(?:得|的)?(?:到|去|往)(?![哪了货付])"
                       ...
                       rf"|青海|宁夏|甘肃|海南|岛上){_C}{{0,6}}?(?:(?:能|可以)?(?:发|送|配送|派送|寄|快递)|能到|到得了)",
     "能不能送到",
