...
- **诉求类型写中文**：质量问题 / 七天无理由 / 发错货（写不认识的词会当场报错，**不猜** ——
  猜错一个词，套用的就是另一条政策）。
- **列名可以换个说法**：这一列叫「退货原因 / 退款原因 / 退货理由 / 原因」都认；金额那列
  叫「退款金额 / 金额」也认。列名是**全等**匹配 —— 认不出来时会指着表头说
  「没有『诉求类型』这一列」并把你写的列名回给你，不会逐行说「第 N 行不能空」。
...
上面几节讲的是**退款**。这个目录里还有第二条通道，形状一样、域不一样：
**供应链付款**（应付账款三单匹配）。分工照旧是「底账配一次 + 申请表每天给」。

| 你是 | 交什么 | 拿到什么 |
| :-- | :-- | :-- |
| 业务方，不写代码 | 一张发票申请表（Excel 另存 CSV），**一个发票明细行一行** | 一张中文结果表：匹配过没过、差在哪、停在哪等谁批、钱到没到账 |
| 发票是图片 | 一叠拍照 / 扫描件 | 先出一张**待复核表**，人逐行确认过才转正式申请表 |

底账这次装的是**供应商、采购订单、收货记录**（真实落地由 ERP 导出）；
申请表里人只填**发票上真实印着的东西**：

```text
发票号,采购单号,行号,SKU,发票数量,发票单价,开票日期,到期日,税率,说明
INV-2026-0001,PO-2026-0001,1,SKU-A,100,12.50,2026-07-25,2026-08-24,0.13,正常
INV-2026-0001,PO-2026-0001,2,SKU-B,50,30.00,2026-07-25,2026-08-24,0.13,
INV-2026-0002,PO-2026-0002,1,SKU-C,80,45.00,2026-08-01,2026-08-31,0.13,数量比收货多 5
```

- 发票号一填，**供应商、币种、订单数量单价、收货数量全部从底账查出来** ——
  与退款域「订单号是唯一那把钥匙」是同一条理由。
- 同一张发票的多行必须共享**采购单号 / 开票日期 / 到期日 / 税率**，
  不一致当场报错，**不取第一行也不取多数**。
- 采购单号认不出就报错，**不做模糊匹配**：`PO-2026-000l` 和 `PO-2026-0001`
  是两张单子，猜错就把钱付给了另一个供应商，而且全程没有一处会变红。

### 图片进件多一道关

```text
发票图片 → 抽取 → 待复核表（多两列：来源 / 需人工确认）→ 人确认 → 正式申请表 → 付款
```

🔴 **只要还有一行「需人工确认=是」，付款入口拒绝整张表。**
读票用的是视觉模型的**实验版**，看错一个小数点就是多付一笔钱 ——
理解可以错，授权不许错。展开讲在 `docs/ap-entry.md`。

### 引擎一行没改

跑的还是 `maos/flows/scenario_10.py` 那条流程（三单匹配 → 付款计划 →
高风险停 `BLOCKED` 等人批 → 发指令 → 轮询回单 → 问不出终态就开对账工单）。
换域只换**入口与数据**，流程、状态机、契约一概不动。

### 现在能跑到哪一步

这条通道的表格入口、票据抽取、复核关口三块并行在建，本节落盘时尚未并入。
冒烟脚本会自己探，缺哪块报哪块并以 **exit 3** 退出（不是 1 —— 「还没装好」
与「装好了但坏了」要分得开）：
