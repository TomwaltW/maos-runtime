"""端到端跑通入口 —— 只做参数解析与分发，场景实现在 maos/flows/scenario_<N>.py。
  ...
  场景 7：退款失败路径  **未落地**，`--scenario 7` 会 ModuleNotFoundError

无参 = 顺跑 1-6（场景 7 未落地，不进缺省序列）。跑完看 event_log：每一次状态迁移都有一条记录，这是后面 Trace 的数据来源。
...
# 缺省序列 = 全部已落地的场景。只排除尚未落地的 7 ——
# 排除标准是「模块不存在」，不是「谁负责」，所以 scenario_7.py 一落地就该进来。
DEFAULT_SCENARIOS = (1, 2, 3, 4, 5, 6)
