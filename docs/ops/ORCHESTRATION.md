...
| Y-1 | **IN_PROGRESS（已被重置）** | worktree `.worktrees/task-y1` @ `42822fc`、分支 `task/y1-exec-path`、0 独有提交、工作区 0 行。11:1x 曾做到「`agents/testing.py` 三个透传点 + `flows/common.py` 两处调用点改完、521 passed 未引红、正要写本轨新测试」后被中断；**11:2x 另一会话重建 worktree 时，那两处未提交改动被冲掉**（本会话实测：`git stash` 空、分支 0 独有提交、新工作区干净 —— **无处可捞**）。派单 `review/paste-Y1.md` 基线已刷到 `42822fc`，**待重粘重做** |
| Y-2 | **NOT_STARTED（已被重置）** | 同上位置 @ `42822fc`、0 独有提交、干净。11:1x 只读了 `skills/invoker.py`/`obs/trace.py`/`kb/experiment.py` 未动任何文件即被中断，**无产出损失**。派单 `review/paste-Y2.md` 基线已刷到 `42822fc`，待重粘 |
| Y-3 | **NOT_STARTED（已被重置）** | 同上 @ `42822fc`、0 独有提交、干净。11:1x 开场自检四条全过、正跑基线 A 组对照时被中断，**无产出损失**。派单 `review/paste-Y3.md` 基线已刷到 `42822fc`，待重粘 |
| Y-4 | **NOT_STARTED（已被重置）** | 同上 @ `42822fc`、0 独有提交、干净。11:1x 开场自检四条全绿（守卫被拦 / 521 passed / 19 passed）、正读关键文件时被中断，**无产出损失**。派单 `review/paste-Y4.md` 基线已刷到 `42822fc`，待重粘 |
| Z-1…Z-5 | **建轨中（另一会话在做）** | 2026-08-29 11:40 实测新出现五个 worktree：`task-z1`(`task/z1-ppt-outline`)、`task-z2`(`z2-demo-cut`)、`task-z3`(`z3-portability-anchor`)、`task-z4`(`z4-checklist-seal`)、`task-z5`(`z5-clone-smoke`)，**全部 @ `42822fc`、0 独有提交**；`review/paste-Z*.md` **尚未落盘**。本行只记「已建轨」这一条实测事实 —— **范围与派单内容由建轨的那个会话补，别照分支名推断它们要做什么** |
